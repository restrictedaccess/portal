...
function search($rt,$key,$status,$p,$maxp,$max) 
{
	...
	//default	
	$where = "WHERE (DATE_FORMAT(e.date_added,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (l.business_partner_id=".$_SESSION['agent_no'].") AND (e.lead_id=l.id) AND (e.userid=p.userid) AND e.status='ON HOLD'";
	...
	{	
		if($key == "")
		{
			...
			$where = "WHERE (DATE_FORMAT(e.date_added,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (l.business_partner_id=".$_SESSION['agent_no'].") AND (e.lead_id=l.id) AND (e.userid=p.userid) AND e.status='$status' ";
			$query = "SELECT $fields $from $where $order_by";
			//ended
		}
		else
		{
			//search w/ keyword
			$where = "WHERE (DATE_FORMAT(e.date_added,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (l.business_partner_id=".$_SESSION['agent_no'].") AND (e.lead_id=l.id) AND (e.userid=p.userid) AND e.status='$status' AND ((p.userid='$key' OR p.email='$key' OR p.fname LIKE '%$key%' OR p.lname LIKE '%$key%') OR (l.id='$key' OR l.email='$key' OR l.fname LIKE '%$key%' OR l.lname LIKE '%$key%'))";
			$query = "SELECT $fields $from $where $order_by";
			//ended			
		}
	}
