...
function CheckCommissionStatus(){
	...
	if (status == 'deleted' || status== 'cancelled' || status == 'pending' || status== 'approved'){
...
function SearchCommissions(e){
	...
	var query = queryString({'status' : status });
...
function OnFailCreateCommission(e){
	alert("There's a problem in creating new commission.");
	$('create_btn').value="Create";
	$('create_btn').disabled=false;
}
