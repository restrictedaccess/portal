...
function refreshScreenshot(){
	...
	var data = {json_rpc:2.0, id:"ID8", method:"get_camshots", params:[today, client_id]}
	    ...
	    success: function(response) {
	    	...
	    	if (screenshots.length>0){
		    	jQuery.each(screenshots, function(i, item){
		    		...
		    		fullscreen_shots.push(STAFF_SERVICE_APP +'camshots/'+item.year+'/'+item.week+'/'+item.id);
		    	})	    		
	    	}
	    	...
	    	if (todisplay.length>0){
		    	...
		    	for(var i=1;i<=screenshots.length;i++){
		    		...
		    		if (i%6==1){
		    			output += '<div class="screen_shot_item"><div class="row"><div class="col-lg-2"><a href="#" data-position="'+i+'" class="thumb_to_full"><img src='+STAFF_SERVICE_APP+'camthumbs/'+item.year+'/'+item.week+'/'+item.id+' alt="Screenshot" class="img-thumbnail" width="100%" data-toggle="tooltip" data-time_to_respond="'+item.time_to_respond+'" data-note="'+item.note+'"/></a><br/>'+item.display_time+'</div>';
		    		}else if (i%6==0){
		    			output += '<div class="col-lg-2"><a href="#" data-position="'+i+'" class="thumb_to_full"><img src='+STAFF_SERVICE_APP+'camthumbs/'+item.year+'/'+item.week+'/'+item.id+' alt="Screenshot" class="img-thumbnail" width="100%" data-toggle="tooltip" data-time_to_respond="'+item.time_to_respond+'" data-note="'+item.note+'"></a><br/>'+item.display_time+'</div></div></div>';		    		
		    		}else{
		    			output += '<div class="col-lg-2"><a href="#" data-position="'+i+'" class="thumb_to_full"><img src='+STAFF_SERVICE_APP+'camthumbs/'+item.year+'/'+item.week+'/'+item.id+' alt="Screenshot" class="img-thumbnail" width="100%" data-toggle="tooltip" data-time_to_respond="'+item.time_to_respond+'" data-note="'+item.note+'"></a><br/>'+item.display_time+'</div>';
		    		}
		    	}
