	    ...
	    success: function(response) {
	    	...
	    	if (screenshots.length>0){
		    	jQuery.each(screenshots, function(i, item){
		    		...
		    		fullscreen_shots.push(STAFF_SERVICE_APP +'shots/'+item.year+'/'+item.week+'/'+item.id+'/'+item.screen_number);
		    	})	    		
	    	}
	    	...
	    	if (todisplay.length>0){
		    	...
		    	for(var i=1;i<=screenshots.length;i++){
		    		...
		    		thumb_image_url = STAFF_SERVICE_APP +"/thumbs/"+item.year+"/"+item.week+"/"+item.id+"/"+item.screen_number;
		    		...
		    		if (i%6==1){
		    			output += '<div class="screen_shot_item"><div class="row"><div class="col-lg-2"><a href="#" data-position="'+i+'" class="thumb_to_full"><img src='+thumb_image_url+' alt="Screenshot" class="img-thumbnail" width="100%" data-toggle="tooltip" data-time_to_respond="'+item.time_to_respond+'" data-note="'+item.note+'"/></a><br/>'+display_time_str+'</div>';
		    		}else if (i%6==0){
		    			output += '<div class="col-lg-2"><a href="#" data-position="'+i+'" class="thumb_to_full"><img src='+thumb_image_url+' alt="Screenshot" class="img-thumbnail" width="100%" data-toggle="tooltip" data-time_to_respond="'+item.time_to_respond+'" data-note="'+item.note+'"></a><br/>'+display_time_str+'</div></div></div>';		    		
		    		}else{
		    			output += '<div class="col-lg-2"><a href="#" data-position="'+i+'" class="thumb_to_full"><img src='+thumb_image_url+' alt="Screenshot" class="img-thumbnail" width="100%" data-toggle="tooltip" data-time_to_respond="'+item.time_to_respond+'" data-note="'+item.note+'"></a><br/>'+display_time_str+'</div>';
		    		}
		    	}
