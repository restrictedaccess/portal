...
function setupContractFromServiceAgreement(service_agreement_id){
	var admin_id = jQuery("#admin_id").val();
	var API_URL = jQuery("#API_URL").val();	
    var url = API_URL+"/activate-prepaid-contract/setup-contract-from-service-agreement/";    		
	var formData = {id : service_agreement_id, admin_id : admin_id};
	var number_of_null_starting_date = jQuery("#number_of_null_starting_date").val();
	
	if(parseInt(number_of_null_starting_date) > 0){
		var starting_date = jQuery("#starting_date").val();
		if(!starting_date){
			alert("Please specify staff starting date.");
			return false;
		}
		formData.starting_date = starting_date;	
	}
	
	//console.log(url);
	if(confirm("Create new contract ?")){
		console.log(formData);	
		jQuery('#create-staff-contract-btn').html('creating...');
		jQuery('#create-staff-contract-btn').attr("disabled", "disabled");
		...
		jQuery.ajax({
			...
			success: function(data){
				...
				jQuery('#create-staff-contract-btn').html('Create New Staff Contract');
				jQuery('#create-staff-contract-btn').removeAttr("disabled");
			},
			error: function(data) {
...
function SendFinalInvoice(){
	...
	var url = PATH + 'create_client_final_invoice/';
	...
	if(confirm("Create Final Invoice?")){
	    jQuery('.btn-final-invoice').html("sending...");
	    jQuery('.btn-final-invoice').attr('disabled', 'disabled');
	    
		
		jQuery.ajax({
            url : url+"?subcon_id="+subcon_id+"&admin_id="+admin_id,
            type : "POST",
            data: formData,
            dataType : 'json',
            success : function(data) {	
                //
                if(data.doc_id){
                	var invoice_url ="/portal/AdminPrepaidInvoice/AdminPrepaidInvoice.html?doc_id="+data.doc_id;
	                jQuery('#windowTitleDialog').modal({ 
						backdrop: 'static',
						keyboard: false
					});
					jQuery('#contract_result').html(data.result);
					...
					if(data.success == true){
						...
						jQuery('#modal_body').addClass("alert-success");
						jQuery('#close_btn').addClass("hide");
						jQuery('#ok_btn').attr("href", invoice_url);
						//jQuery('#ok_btn').attr("target", "_blank");
					}else{
						jQuery('#modal_body').removeClass("alert-success");
						jQuery('#modal_body').addClass("alert-danger");
						jQuery('#close_btn').removeClass("hide");
				...
				jQuery('.btn-final-invoice').html('Send Final Invoice To Client');
				jQuery('.btn-final-invoice').removeAttr('disabled', 'disabled');
            },
            error : function(data) {
                alert("There's a problem in sending client final invoice.");
				jQuery('.btn-final-invoice').html('Send Final Invoice To Client');
				jQuery('.btn-final-invoice').removeAttr('disabled', 'disabled');
            }
        });
...
function reloadAcceptedServiceAgreementList(){
	jQuery("#page").val(1);
	getAcceptedServiceAgreementsFromDb();
}

function getAcceptedServiceAgreementsFromDb(){
	var base_api_url = jQuery("#base_api_url").val();	
    var url = base_api_url+"/service-agreement/get-accepted-service-agreements-from-db/";    
    var page = jQuery("#page").val();		
	var formData = {page : page};
	
	jQuery("#contact_container ul").html("<li class='alert alert-info'>Loading. Please wait....</li>");
	jQuery.get( url, formData, function(response){
        response = jQuery.parseJSON(response);
        if(response.success){
            var output="";
            jQuery.each(response.result, function(i, item) {
                output +="<li style='padding:10px;'>";
					if(item.total_subcons != "0"){
						output += "<a  href='/portal/django/subcontractors/service_agreement_setup/"+item.service_agreement_id+"' target='_blank' style='float:right;'><span class='label label-info'>"+item.total_subcons+"</span> Staff Contract Created</a>";
					}                			
                output += item.counter +") ";			
                output +="<a href='/portal/django/subcontractors/service_agreement_setup/"+item.service_agreement_id+"' target='_blank'>";		
     			output +="<strong><u>#"+item.service_agreement_id+" Service Agreement</u></strong></a><br />";  
     			output +="Client : #"+item.client.id+" "+item.client.fname+" "+item.client.lname+"<br />";
     			output +="Date Accepted : "+ item.date_accepted+"<br>";
     			
     			if(item.details){
     				output +="<ol class='service_agreement_details'>";
     					jQuery.each(item.details, function(d, detail) {
     						output +="<li>";
     						output += detail.service_agreement_details;
     						output +="</li>";
     					});
     				output +="</ol>";
     			}
            ...
            jQuery("#contact_container ul").html(output);
            jQuery("#sa_result_str").html(response.numrows +" records found");
...
function set_up_pagination(pagenum, maxpage){
	...
	if (pagenum > 1){
		page = pagenum - 1;
		output += "<a href='#' data-page-num="+page+">Previous</a> ";			
		output += "<a href='#' data-page-num=1>First</a> ";
	}
	
	output += " Page "+pagenum+" of "+maxpage+" ";
	
	if (pagenum < maxpage){
		page = pagenum + 1;
		output += "<a href='#' data-page-num="+page+">Next</a> ";
		output += "<a href='#' data-page-num="+maxpage+">Last</a> ";
	}
	
	jQuery(".sa-pagination").html(output );	
	jQuery(".sa-pagination a").click(function(e){
		e.preventDefault();
...
function show_working_sched(){
	var update_working_sched = jQuery('#update_working_sched').val();
	var subcon_id = jQuery('#subcon_id').val();
	var work_status = jQuery('#work_status').val();
	jQuery("#daily_wok_schedule_container").addClass("hide");
	if(update_working_sched == "yes"){
		jQuery("#daily_wok_schedule_container").removeClass("hide");
		jQuery("#daily_wok_schedule_container").html("Loading staff working schedules...");
		
		//console.log(query)
		var url = PATH + 'show_working_sched/';
		...
		result.done(function( data ) {
			jQuery("#daily_wok_schedule_container").html(data);
			CheckWeekDays();
			ConfigureStaffSchedule();
			jQuery('input[name=weekday]').click(function() {
				CheckedUncheckedWeekday(this);
			});
			
			jQuery('#schedule select').change(function(e){
				GetTimeDifference(this);
			});
...
function GetStaffWorkSchedule(){
	var work_status = jQuery('#work_status').val();
	var recorded_work_status = jQuery('#work_status').attr("work_status");
	//jQuery("#show_working_sched_btn").addClass("hide");
	jQuery("#daily_wok_schedule_container").addClass("hide");
	jQuery('#update_working_sched').val("no");
	if(work_status != recorded_work_status){
		//jQuery("#show_working_sched_btn").removeClass("hide");
		jQuery('#update_working_sched').val("yes");
		show_working_sched();
	}
}
...
function ScheduleUpdatedRates(){
	...
	var subcon_id = jQuery('#subcon_id').val();
	var update_working_sched = jQuery("#update_working_sched").val();
	var comment_note = jQuery("#comment_note").val();
	...
	if(confirm("Save changes?")){
		var query = {"subcon_id": subcon_id, "client_price" : client_price, "work_status" : work_status, "php_monthly" : php_monthly, "scheduled_date" : scheduled_date, "comment_note" : comment_note};
		//console.log(query);
		//return false;
		...
		jQuery.ajax({
			...
			success: function(data){
				...
				if(update_working_sched == 'yes'){
					//console.log(update_working_sched);
					UpdateWorkingSched();
				}else{
					jQuery('#windowTitleDialog').modal({ 
						backdrop: 'static',
	...
	jQuery.ajax({
		...
		success: function(data){
			...
			jQuery.each(histories, function(i, item){	
				...
				if(item.note){
					output += '<div><strong>Comment/Note</strong> : <em>'+item.note+'</em></div>';	
				}	
...
function CancelContract(e){
	...
	var replacement_request = jQuery('#replacement_request').val();
	var admin_notes = jQuery('#admin_notes').val();
	var pay_hrs_to_staff = jQuery('#pay_hrs_to_staff').val();
	var pay_hrs_to_staff_start_date = jQuery('#pay_hrs_to_staff_start_date').val();
	var pay_hrs_to_staff_end_date = jQuery('#pay_hrs_to_staff_end_date').val();
	var admin_id = jQuery("#admin_id").val();
	...
	var query = {
		...
		'pay_hrs_to_staff_start_date' : pay_hrs_to_staff_start_date,
		'pay_hrs_to_staff_end_date' : pay_hrs_to_staff_end_date
	};
	console.log(query);
	...
	if(confirm("Cancel contract?")){
		...
		*/
		...
		jQuery.ajax({
            url : API_URL + "/subcontractors/schedule-staff-contract-termination/",
            type : "POST",
            data: query,
            dataType : 'json',
            success : function(data) {	
...
function ApproveUpdatedStaffContract(){
	...
	var service_agreement_id = jQuery('#service_agreement_id').val();
	var quote_details_id = jQuery('#quote_details_id').val();
	var extra_hours = jQuery("#extra_hours").val();
...
function CreateNewTempContract(){
    ...
    var margin_value = jQuery('#margin_value').val();
    var extra_hours = jQuery('#extra_hours').val();
	...
	jQuery.ajax({
		...
		success: function(data){
			...
			if(data.success == true){
				...
				//For AJAX activation of Staff contract. 
				//Start Send process to beanstalk
				var base_api_url = jQuery("#base_api_url").val();	
			    var url = base_api_url+"/activate-prepaid-contract/activate/";    			    		
				var formData = {id : data.temp_id, admin_id : data.admin_id};
				...
				//End sending process to beanstalk
				*/
			}else{
				jQuery('#modal_body').removeClass("alert-success");
				jQuery('#modal_body').addClass("alert-danger");
...
function ConfigurePrice(obj){
	var staff_currency_id = null;
	var staff_currency = null;
	var work_status = jQuery('#work_status').val();
	var price = jQuery('#'+obj).val();
	console.log(obj);
	console.log("-------");
	console.log(staff_currency_id);
	if (obj.trim() == "php_monthly") {
		staff_currency_id = jQuery("#staff_currency_id").val().trim();

        if (staff_currency_id == 3) {
            staff_currency = '($)AUD';
        } else if (staff_currency_id == 4) {
            staff_currency = '(&pound;)GBP';
        } else if (staff_currency_id == 5) {
            staff_currency = '($)USD';
        } else if (staff_currency_id == 6) {
            staff_currency = '(&#8369;)PHP';
        } else if (staff_currency_id == 8) {
            staff_currency = '(&#8377;)INR';
        }
	}else {
        staff_currency_id = jQuery("#currency").val().trim();

        if (staff_currency_id == "AUD") {
            staff_currency = '($)AUD';
		} else if (staff_currency_id == "GBP") {
            staff_currency = '(&pound;)GBP';
        }else if (staff_currency_id == "USD") {
            staff_currency = '($)USD';
        }
	}
	...
	if(price !=""){
		...
		jQuery('#'+obj+'_str').html("<br /><span class='badge' style='margin-left: 16%;'>Yearly : "+staff_currency+ ' ' +formatNumber(yearly)+"</span> <span class='badge'>Monthly : "+staff_currency+ ' ' + formatNumber(price)+ "</span> <span class='badge'>Weekly : "+staff_currency+ ' '  + formatNumber(weekly)+ "</span> <span class='badge'>Daily : "+staff_currency+ ' ' + formatNumber(daily) +"</span> <span class='badge'>Hourly : "+staff_currency+ ' ' +formatNumber(hourly)+"</span>");
		
	}else{
		jQuery('#'+obj+'_str').html("");
		jQuery('#'+obj+'_str').addClass("hide");
	}
	jQuery(".badge").css('margin-bottom', '10px');
	GetRevenue();
}
...
function DisplayWorkingHoursDaysBreakTime(){
	...
	if(isNaN(total_working_hours)){
		total_working_hours=0;
	}
	if(isNaN(total_break_time_hours)){
...
function search_subconlist_reporting(){
	...
	if(include_inhouse_staff == 'no'){
		for(var i = optionValues.length-1; i--;){
			if (optionValues[i] === "11") optionValues.splice(i, 1);
		}	
	}
...
function CheckCSRO(){
	...
	jQuery('#include_inhouse_staff').removeAttr("disabled", "disabled");
	
	if((!isNaN(active_client) ) || (!isNaN(inactive_client) )){
		jQuery('#csro_id').attr("disabled", "disabled");
		jQuery('#csro_id').val("");
		
		jQuery('#include_inhouse_staff').attr("disabled", "disabled");
		jQuery('#include_inhouse_staff').val("");
	}
