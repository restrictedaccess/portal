...
jQuery(document).ready(function(){
	...
	var registered_date = jQuery("#client_manager_registered_date").val();
	
	var splitted_registered_date = registered_date.split(" ");
	
	var date_month = splitted_registered_date[0];
	
	var date_day = splitted_registered_date[1].replace(",", "");
	
	var date_year = splitted_registered_date[2].replace(",", "");
	
	var new_registered_date = date_month + " " + date_day + " " + date_year;
	
	init_reset_password_first("manager", new_registered_date);
	    	...
	    	if (response.result!=undefined){
		    	...
		    	if (todisplay.length>0){
			    	...
			    	for(var i=1;i<=4;i++){
			    		...
			    		if (i%2==1){
	                        output += '<div class="screen_shot_item"><div class="row"><div class="col-lg-6"><a href="/portal/django/manager_productivity_reports/screenshots/"><img src="/portal/django/client_subcon_management/thumbs/'+item.year+'/'+item.week+'/'+item.id+'/'+item.screen_number+'" alt="Screenshot" class="img-thumbnail" width="100%"/></a></div>';
                        }else{
                            output += '<div class="col-lg-6"><a href="/portal/django/manager_productivity_reports/screenshots/"><img src="/portal/django/client_subcon_management/thumbs/'+item.year+'/'+item.week+'/'+item.id+'/'+item.screen_number+'" alt="Screenshot" class="img-thumbnail" width="100%"/></a></div></div></div>';
                        }
