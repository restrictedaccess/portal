...
if ($sub_category_id){
    ...
    $sql1 = $db->select()->from("job_position_skills_tasks", array("id", "value", "sub_category_id"))->where("sub_category_id=".$sub_category_id." AND value LIKE '%".$search_task."%' AND type='task' AND display_website='YES'");
    $sql2 = $db->select()->from("job_position_skills_tasks", array("id", "value", "sub_category_id"))->where("sub_category_id=".$sub_category_id." AND value LIKE '%".$search_task."%' AND type='task' AND leads_id = ".$_SESSION["leads_id"]);
