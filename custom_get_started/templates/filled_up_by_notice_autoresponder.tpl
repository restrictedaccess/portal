...
<div style='font-family:Arial, Helvetica, sans-serif; font-size:14px'>
	...
	<p>Created and Filled up by <strong>{$filled_up_by}</strong> on {$ATZ|date_format:"%B %e, %Y %I:%M:%S %p"}</p>
	<p>Lead : #{$lead.id} {$lead.fname} {$lead.lname} </p>
	<p>Email : {$lead.email}</p>
	<!-- removed link on job spec form -->
	{*<p>Order ID : <a href='http://{$site}/portal/ShowRecruitmentServiceOrder.php?order_id={$leads_invoice_id}'>http://{$site}/portal/ShowRecruitmentServiceOrder.php?order_id={$leads_invoice_id}</a></p>*}
