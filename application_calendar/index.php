...

{
...

	{
		...
		$error_message = "";
		mysql_query("INSERT INTO tb_app_appointment(user_id, leads_id, client_id, date_start, date_end, start_month, end_month, start_day, end_day, start_year, end_year, start_hour, start_minute, end_hour, end_minute, subject, location, description, appointment_type, is_allday, is_any, status, date_added) VALUES('$assign_to', '$leads_id', '$client_id', '$date_start', '$date_end', '$start_month', '$end_month', '$start_day', '$end_day', '$start_year', '$end_year', '$start_hour', '$start_minute', '$end_hour', '$end_minute', '$subject', '$location', '$description', '$type', '$all_day', '$any', '$status', '$date_added')");					
		...
		{		
			...
			$body=$h."
			<body>
			<p>".$applicant_full_name.", you are invited ";
			...
			{
				$body = $body."to </p><h2>Location: ".$location."</h2>";
			}
			$body = $body."<p>".$date_start." ".$s_hour.":".$start_minute.$start_hour_type." - ".$date_end." ".$e_hour.":".$end_minute.$end_hour_type." <br>
			  (Timezone: Philippines) <br>
			  <br>
			...
			$body=$h."
			<body>
			<p>".$client_full_name.", you are invited ";
			if($location == "" || $location == NULL)
			{
				$body = $body."for a meeting.</p>";
			}
			...
			<table width="100%" border="0" bgcolor="#666666" cellpadding="10" cellspacing="1">
				...
				<?php } ?>
										...
										<?php
											...
											$c = mysql_query("SELECT id FROM tb_app_calendar_actions WHERE angent_id='$a_id'");	
				...
				<tr>
					...
					<td width="5%" align="left" valign="top" bgcolor="#FFFFFF"><strong><font color="#000000">Actions(<?php echo $num_result; ?>)</font></strong></td>
					...
					<td width="24%" align="left" valign="top" bgcolor="#D9D902">
											...
											<?php	
												$db=connsql();
												$c = mysql_query("SELECT * FROM tb_app_calendar_actions WHERE angent_id='$a_id' ORDER BY date_added DESC LIMIT 5");	
												while ($row = @mysql_fetch_assoc($c)) 
												{
													$id = $row["id"];
													
													$appointment_id = $row["appointment_id"];
													$ag = mysql_query("SELECT leads_id, subject, status FROM tb_app_appointment WHERE id='$appointment_id'");
														...
														<tr>
															<td valign="top">
																<table width=100% cellspacing=1 border=0 cellpadding=7 bgcolor=#fefefe>
																	<tr>
																		<td onMouseOver="javascript:this.style.background='#F1F1F3'; " onMouseOut="javascript:this.style.background='#ffffff'; " onClick="javascript: view_actions(<?php echo @$row["appointment_id"]; ?>); " bgcolor=#FFFFFF valign="top" width="80%"><?php echo @$subject."&nbsp;(Type: ".$type.")"; if(@$stat=="not active") { echo "(Status: this meeting is finished)"; } elseif(@$stat=="active") { echo "(Status: waiting)"; } else { echo "(Status: meeting cancelled)"; }?></td>
											...
											?>
														<tr>
															<td valign="top">
																<table width=100% cellspacing=1 border=0 cellpadding=7 bgcolor=#fefefe>
																	<tr>
																		<td onMouseOver="javascript:this.style.background='#F1F1F3'; " onMouseOut="javascript:this.style.background='#ffffff'; " onClick="javascript: view_all_actions(); " bgcolor=#FFFFFF valign="top" width="80%" colspan="2"><a href="javascript: view_all_actions(); ">View All Actions</a></td>
																	</tr>
																</table>
															</td>
														</tr>									
...
<?php
					...
					$get_time = mysql_query("SELECT id, start_hour, end_hour, subject FROM tb_app_appointment WHERE user_id='$user_id' AND date_start='$date_selected' AND is_allday='yes' ORDER BY start_hour ASC");
...
<?php	
						...
						$get_time = mysql_query("SELECT id, start_hour, end_hour, start_minute, end_minute, subject, description FROM tb_app_appointment WHERE user_id='$user_id' AND date_start='$date_selected' AND is_allday='no' AND start_hour >= '$hrs' AND start_hour < '$sub_hrs' ORDER BY start_minute ASC");
