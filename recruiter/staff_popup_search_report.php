...
$sql = "SELECT id, fname, lname FROM leads 
WHERE id='$keyword' OR email='$keyword' OR (fname LIKE '%$keyword%' OR lname LIKE '%$keyword%')";
...
foreach($result as $rows)
{
			...
			<tr>
				...
				<td align="left" valign="top" class="td_info"><input type="radio" name="lead_id" onchange="javascript: document.getElementById(\'search_lead_id\').value='.$rows['id'].'; hide_popup_report(); " />'.$name.'&nbsp;<font size=1><em>('.$rows['id'].')</em></font></td>
			</tr>
			';
}
