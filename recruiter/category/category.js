...
//START - staff timezone
function staff_timezone(userid,id,t,type)
{
	
	if (jQuery("#tz4").is(":checked")){
		jQuery("#tz3").attr("disabled", "disabled");
		jQuery("#tz2").attr("disabled", "disabled");
		jQuery("#tz1").attr("disabled", "disabled");
	}else{
		jQuery("#tz3").removeAttr("disabled");
		jQuery("#tz2").removeAttr("disabled");
		jQuery("#tz1").removeAttr("disabled");
	}
	
	
	if (jQuery("#tz_pt4").is(":checked")){
		jQuery("#tz_pt3").attr("disabled", "disabled");
		jQuery("#tz_pt2").attr("disabled", "disabled");
		jQuery("#tz_pt1").attr("disabled", "disabled");
		
	}else{
		jQuery("#tz_pt3").removeAttr("disabled");
		jQuery("#tz_pt2").removeAttr("disabled");
		jQuery("#tz_pt1").removeAttr("disabled");
	...
	{
		if(document.getElementById("tz_pt"+id).checked == true)
		{
			ac = "check";
		}
		else
	...
	xmlHttp.open("GET","staff_timezone_update.php?type="+type+"&time_zone="+t+"&action="+ac+"&userid="+userid,true);
	xmlHttp.send(null);
}

function staff_time(userid)
{
	var time_entered = document.getElementById('start_hour_id').value+":"+document.getElementById('start_minute_id').value+":00";
	xmlHttp=GetXmlHttpObject();
	xmlHttp.open("GET","staff_timezone_update.php?time="+time_entered+"&type=time&userid="+userid,true);
	xmlHttp.send(null);
	alert("The Time has been set.");
}
//ENDED - staff timezone
...
function saveEvaluation(){
	...
	//Hot
	if(document.getElementsByName("hot")[0].checked==true){
		hot = "yes";
	}else{
		hot = "no";
	}
	...
	//Experienced
	if(document.getElementsByName("experienced")[0].checked==true){
		experienced = "yes";
	}else{
		experienced = "no";
	}	
	...
	url=url+"&hot="+hot;
	url=url+"&experienced="+experienced;
	//url=url+"&notes="+notes;
	url=url+"&ran="+Math.random();
...
function onSuccessSaveEvaluation(){
	...
	{
		document.getElementById("container").innerHTML="<img src='../images/ajax-loader.gif'>";
	}	
	//if (xmlHttp.readyState==4)
	//{ 	
		//showEvaluationForm();
...
function showCategoriesAddForm(userid){
	//userid = document.getElementById("userid").value;
	if(userid==""){
...
function getSubAllCategories(category_id){
	...
	var url=PATH+"getAllSubCategory.php";
	url=url+"?category_id="+category_id;
	url=url+"&ran="+Math.random();
...
function addSubCategory(){
	...
	if(classification == ""){
		alert("Please specify a classification!");
		return false;
	}
...
function getAllCategory(){
	...
	var url=PATH+"getAllCategory.php";
	url=url+"?ran="+Math.random();
...
//ROY CODES --------------------------------------------------------------------------------------------------------------------------------------------------
//START - add evaluation comment
function addComment(form_obj)
{
	...
	{
		var data = jQuery(form_obj).serialize();
		jQuery.post("/portal/recruiter/category/addEvaluationComment.php", data, function(response){
			//alert("You notes has been successfully added.");
			jQuery("#notes").val("");
