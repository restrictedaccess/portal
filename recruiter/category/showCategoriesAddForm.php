...
$data=mysqli_query($link2,$queryApplicant);
...
?>
	...
	<?
	$queryAllJobCategories = "SELECT category_id, category_name FROM job_category j WHERE (j.status = 'posted' OR j.category_id=60);";
	$result = mysqli_query($link2,$queryAllJobCategories);
	$counter = 0;
	while(list($category_id, $category_name)=mysqli_fetch_array($result))
	{
		$counter++;
