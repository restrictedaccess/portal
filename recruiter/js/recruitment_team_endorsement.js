function renderResponse(response){
	...
	jQuery.each(response.rows, function(i, item){
		...
		link = "/portal/recruiter/get_endorsed_staff.php?posting_id="+item.posting_id+"&tracking_code="+item.tracking_code+"&shortlist_type="+jQuery("select[name=shortlist_type]").val();
		row += "<td><a href='"+link+"' class='popup'>"+item.total_shortlist+"</a></td>";	
		jQuery.each(item.all_recruiters, function(j, recruiter){
			link = "/portal/recruiter/get_endorsed_staff.php?posting_id="+item.posting_id+"&tracking_code="+item.tracking_code+"&shortlist_type="+jQuery("select[name=shortlist_type]").val()+"&admin_id="+recruiter.admin_id;
...
jQuery(document).ready(function(){
	...
	if (!hasRequest){
		addPreloader();
		hasRequest = true;
		jQuery.get("/portal/recruiter/load_jo_endorsement.php", function(response){
	...
	jQuery("#filter-form").submit(function(){
		if (!hasRequest){
			...
			jQuery.get("/portal/recruiter/load_jo_endorsement.php?"+data, function(response){
				response = jQuery.parseJSON(response);
				renderResponse(response);		
			})
			jQuery("#total_shortlists tbody").html("");
			
			jQuery.get("/portal/recruiter/load_recruitment_team_endorsements_total.php?"+data, function(response){
				response = jQuery.parseJSON(response);
				var output = ""
				var totalAssigned = 0;
	...
	jQuery.get("/portal/recruiter/load_recruitment_team_endorsements_total.php", function(response){
		response = jQuery.parseJSON(response);
		var output = ""
		var totalAssigned = 0;
		var totalUnassigned = 0;
