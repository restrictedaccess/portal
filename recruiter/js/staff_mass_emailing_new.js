		...
		//START: sending email
			...
			function send_now(form_obj) 
			{
				...
				var message = jQuery("#message_wrapper").text();
				if (jQuery.trim(subject)==""){
					alert("Subject is required for sending email");
					return;
				}
				if (jQuery.trim(message)==""){
					alert("Message is required for sending email");
					return;
				}
				
				var answer = confirm("Do you want to send this email to the selected staff?");
				if (answer){
					document.getElementById('sending_icon').innerHTML = '<img src="images/sending.gif" />';
					poststr = jQuery(form_obj).serialize();
			...
			function sending() 
			{
				if(sending_status == "on")
				{
					if(server_response_status == "off")
					{
						...
						staff_mass_email_obj.open("POST", 'staff_mass_emailing_sending_new.php', true);
		...
		{
			var status;
			sent_items_obj.open('get', 'staff_search_items_new.php?'+path)
		...
		{
			var status;
			waiting_items_obj.open('get', 'staff_search_items_new.php?'+path)
...
jQuery(document).ready(function(){
	jQuery("#message_wrapper").each(function(){
		this.contentEditable = true;
	});
	jQuery("#template_selector").change(function(){
		var selectedValue = jQuery(this).val();
		var source = "";
		if (selectedValue=="blank"){
			source = "";
			jQuery("#subject").val("");
		}else if (selectedValue=="your_job_application"){
			source = jQuery("#application_interested").html();
			jQuery("#subject").val("Do we still profile your resume on our Remote Staff website");
		}
		jQuery("#message_wrapper").html(source);
	})
	
	function updateMessage(){
		jQuery("#message").val(jQuery("#message_wrapper").html());
	}
