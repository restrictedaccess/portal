...
jQuery(document).ready(function(){
	...
	function filterGrid(openCloseCancel, serviceType){
		...
		var url = "/portal/recruiter/load_job_postings.php?";
		...
		if (filter_type==0&&serviceType==undefined&&openCloseCancel==undefined&&keyword==""&&recruiter==""&&hiringcoordinator==""){
			url = "/portal/recruiter/load_job_postings.php?display="+displayStatus+"&";
		}
		if (grid!=null){
			grid.reload(url);	
		}
	...
	function renderRecruiter(value, options, row){
		if (credentials.status == "FULL-CONTROL"||credentials.admin_id==100){
			
			var output="";
			var link = "";
			if (row.gs_job_role_selection_id!=""){
				output+="<div id='recruiters-"+row.gs_job_titles_details_id+"-JO'>";
				link="<a href='' class='add-recruiters' data-id='"+row.gs_job_titles_details_id+"' data-type='JO'>Add New Recruiter</a>";
			}else{
				output+="<div id='recruiters-"+row.leads_id+"-Lead'>";
				link="<a href='' class='add-recruiters' data-id='"+row.leads_id+"' data-type='Lead'>Add New Recruiter</a>";
			}
			...
			jQuery.each(row.assigned_recruiters, function(i, item){
				...
				var select = "<select class='hr_update hr_select' data-link-id='"+link_id+"' data-link-type='"+link_type+"' data-recruiter-id='"+hr_id+"'>";
				var deleteLink = "<span id='hr_delete-"+link_id+"-"+link_type+"-"+hr_id+"' class='ui-icon ui-icon-close hr_delete' data-link-id='"+link_id+"' data-link-type='"+link_type+"' data-recruiter-id='"+hr_id+"'></span>";
	...
	function renderHiringCoordinators(value, options, row){
		if (credentials.status == "FULL-CONTROL"){
			...
			var select = "<select "+selectClass+" "+id+">";
	...
	function renderServiceType(value, options, row){
		if (credentials.status == "FULL-CONTROL"){
			...
			var disabled = "";
			/*
			if (row.gs_job_role_selection_id==""||value=="ASL"){
				disabled="disabled";
			}
			*/
			...
			*/
			var select = "<select class='select_hr_service_update' id='"+selectId+"' "+disabled+" "+data_value+" "+data_id+" "+data_status+" "+data_merged_order_id+" "+data_old_service_type+" "+data_date_added+" "+data_lead_id+" "+data_jsca_id+">";
	...
	jQuery(".add-recruiters").live("click", function(e){
		...
		var select = "<select id='hr_update-"+link_id+"-"+link_type+"' class='hr_update hr_select' data-link-id='"+link_id+"' data-link-type='"+link_type+"'>";
	...
	var unmergeTo = jQuery("#unmerge-to").button({
		...
		if (!hasNoMerge){
			...
			if (answer){
				...
				jQuery.post("/portal/recruiter/unmerge_order.php", {merged_order_ids:selectedMergeOrderIds}, function(data){
					data = jQuery.parseJSON(data);
					if (data.success){
			...
			jQuery.get("/portal/recruiter/get_all_recruiters.php?hr=1", function(data){
				...
				if (credentials.status == "FULL-CONTROL"){
					grid = new RSInvertedGrid({
						columns:["Action", "Tracking Code",'Job Title',"Job Specification Link",'Client',"Last Contact", "Number Of Staff Needed", "Gap on Endorsed", "Gap on Interviewed", "View History", "View HM Notes", "Service Type", "Order Status",'Work Status', 'Date Ordered', 'Date When Staff is Required', 'Order Age In Days', 'Work Time Zone', "Budget", "Notes and Comments", "Recruiter", "Hiring Coordinator", "Business Developer",  "Shortlisted", "Endorsed / <i>ASL Interview Request</i>", "Interviewed",  "Cancelled","Hired", "On Trial","Rejected"],
						id:"recruitment-sheet",
						url:"/portal/recruiter/load_job_postings.php",
						pager:"#pager",
						...
						colModel:[{name:"action", align:"center", width:200,sortable:false, formatter:renderAction},
						          	...
						          	{name:"average_endorsed", sortable:false, frozen:true},
						          	{name:"average_interviewed", sortable:false, frozen:true},
					...
					grid = new RSInvertedGrid({
						columns:["Tracking Code", 'Job Title',"Job Specification Link", "View History", "View HM Notes", "Gap on Endorsed", "Gap on Interviewed",  'Client', "Last Contact","Number Of Staff Needed", "Service Type", "Order Status", 'Work Status', 'Date Ordered', 'Date When Staff is Required', 'Order Age In Days', 'Work Time Zone', "Budget", "Notes and Comments", "Recruiter", "Hiring Coordinator", "Business Developer",  "Shortlisted", "Endorsed / <i>ASL Interview Request</i>", "Interviewed", "Cancelled","Hired", "On Trial","Rejected"],
						id:"recruitment-sheet",
						url:"/portal/recruiter/load_job_postings.php",
						pager:"#pager",
						dataLoaded:dataLoaded,
