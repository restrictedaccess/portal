...
function getApplicants($posting,$page,$date_start,$date_end,$updated_date_start,$updated_date_end,$recruiter,$keyword,$keyword_type,$country,$recruitment_team_id){  
	...
	if (isset($_GET["display"])&&$_GET["display"]!=""){
		$asl_filter = " and jsca.ratings=".addslashes($_GET["display"])." ";
	}
	...
	$inactive_filter_categorized = " and pe.userid NOT IN (SELECT userid FROM inactive_staff) ";
	...
	if( $posting != ''){
		$select_start = 'select SQL_CALC_FOUND_ROWS a.userid,p.id,a.status,c.latest_job_title,ad.admin_fname,ad.admin_lname,
			...
			right join job_sub_category_applicants as jsca on jsca.userid=pe.userid ';
		...
		$select_start = 'select SQL_CALC_FOUND_ROWS c.latest_job_title,ad.admin_fname,ad.admin_lname,
			...
			LEFT JOIN personal AS pe on jsca.userid=pe.userid 
	...
	$select = "
		...
		".$posting_filter.$availability_filter.$reg_date_filter.$update_date_filter.$recruiter_filter.$keyword_filter.$country_filter.$region_filter.$city_filter.$asl_filter.$inactive_filter_categorized."
		group by pe.userid 
