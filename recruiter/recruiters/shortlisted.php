...
function getApplicants($posting,$page,$date_start,$date_end,$updated_date_start,$updated_date_end,$recruiter,$keyword,$keyword_type,$country, $recruitment_team_id){  
	...
	if(($updated_date_start != '')&&($updated_date_end != '')){
		$update_date_filter = " and date(ss.date_listed) >=date('".$updated_date_start."') and date(ss.date_listed) <= date('".$updated_date_end."')" ;
	}
	/*
	$update_date_filter = '';
	if(($updated_date_start != '')&&($updated_date_end != '')){
	...
	*/
	...
	if( $posting != ''){
		$select_start = 'select SQL_CALC_FOUND_ROWS p.id,c.latest_job_title,ad.admin_fname,ad.admin_lname,
			...
			from tb_shortlist_history ss 
			LEFT JOIN posting as p ON ss.position = p.id 
			left join personal as pe FORCE INDEX (PRIMARY) on ss.userid=pe.userid 
		...
		$select_start = 'select SQL_CALC_FOUND_ROWS c.latest_job_title,ad.admin_fname,ad.admin_lname,
			pe.userid,pe.fname,pe.lname,pe.email,pe.voice_path,pe.skype_id,pe.yahoo_id,pe.datecreated,pe.dateupdated,pe.permanent_residence, 
			ss.id AS shortlist_id 
			from tb_shortlist_history ss  
		    left join personal as pe FORCE INDEX (PRIMARY) on pe.userid=ss.userid 
		    left join personal_user_logins AS pu on pu.userid = pe.userid 
		    left join currentjob as c on pe.userid=c.userid ';
		$selectCountQuery = "SELECT pe.userid 
			from tb_shortlist_history ss  
	...
	$select = "
		...
		".$login_filter.$posting_filter.$availability_filter.$reg_date_filter.$update_date_filter.$recruiter_filter.$keyword_filter.$country_filter.$inactiveUserFilter.$region_filter.$city_filter."
		group by pe.userid 
	...
	foreach ($results as $key=>$result){ 
		...
		$shortlist = $db->fetchRow($db->select()->from(array("ss"=>"tb_shortlist_history"), array("ss.date_listed AS date_listed"))->where("ss.userid = ?", $result["userid"])->order("ss.date_listed DESC")->limit(1));
		$results[$key]["date_listed"] = date("Y-m-d", strtotime($shortlist["date_listed"]));
...
$result = getApplicants($posting,$page,$date_start,$date_end,$updated_date_start,$updated_date_end,$recruiter,$keyword,$keyword_type,$country, $recruitment_team_id);
...
$smarty->display('recruiter_search_shortlisted.tpl'); 