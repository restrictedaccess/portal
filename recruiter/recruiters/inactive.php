...
function getApplicants($posting,$page,$date_start,$date_end,$updated_date_start,$updated_date_end,$recruiter,$keyword,$keyword_type,$country, $recruitment_team_id){  
	...
	*/
	...
	if (isset($_GET["inactive"])&&($_GET["inactive"]!="")){
		$inactive_filter = " and ina.type = '".$_GET["inactive"]."'";
	}
	...
	if( $posting != ''){
		$select_start = 'select SQL_CALC_FOUND_ROWS a.userid,p.id,a.status,c.latest_job_title,ad.admin_fname,ad.admin_lname,
			...
			inner join inactive_staff as ina on ina.userid = pe.userid ';
		$selectCountQuery = "SELECT pe.userid from posting as p
			left join applicants as a on p.id=a.posting_id
			left join personal as pe FORCE INDEX (PRIMARY) on a.userid=pe.userid 
			left join personal_user_logins AS pu on pu.userid = pe.userid 
			left join currentjob as c on a.userid=c.userid 
		...
		$select_start = 'select SQL_CALC_FOUND_ROWS c.latest_job_title,ad.admin_fname,ad.admin_lname,
			...
			from inactive_staff ina 
		    left join personal as pe FORCE INDEX (PRIMARY) on pe.userid=ina.userid 
		    left join personal_user_logins AS pu on pu.userid = pe.userid 
		    left join currentjob as c on pe.userid=c.userid ';
		$selectCountQuery = "SELECT pe.userid from inactive_staff ina 
		    left join personal as pe FORCE INDEX (PRIMARY) on pe.userid=ina.userid 
		    left join personal_user_logins AS pu on pu.userid = pe.userid 
		    left join currentjob as c on pe.userid=c.userid ";
	...
	$select = "
		...
		".$login_filter.$posting_filter.$availability_filter.$reg_date_filter.$update_date_filter.$recruiter_filter.$keyword_filter.$country_filter.$inactive_filter.$region_filter.$city_filter."
		group by pe.userid 
	...
	foreach ($results as $key=>$result){ 
		...
		$results[$key]['inactive_type'] = $db->fetchOne($db->select()->from(array("ina"=>"inactive_staff"), array("type"))->where("ina.userid = ?",$results[$key]['userid']));
...
for($i=0; $i<count($ph_states_array); $i++){
	if($selected_region == $ph_states_array[$i]){
...
if ($staff_status=="INACTIVE"){
	$smarty->assign("inactiveSelect", $inactiveSelect);
}
//availability inputs
