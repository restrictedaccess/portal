...
$remote_ready_checked = $_GET["remove_remote_ready"];
$over_30 = $_GET["over_30"];
...
function getApplicants($posting,$page,$date_start,$date_end,$updated_date_start,$updated_date_end, $date_progress_start, $date_progress_end, $date_progress_tagged_start, $date_progress_tagged_end, $recruiter,$keyword,$keyword_type,$country, $recruitment_team_id){  
	...
	*/
	...
	/* Injected By Josef Balisalisa
	if (isset($_GET["remove_remote_ready"])&&$_GET["remove_remote_ready"]){
		$sql = "SELECT rrce.userid AS userid FROM remote_ready_criteria_entries AS rrce LEFT JOIN remote_ready_criteria AS rrc ON rrc.id = rrce.remote_ready_criteria_id INNER JOIN personal AS pe ON pe.userid=rrce.userid WHERE (pe.image IS NOT NULL or pe.voice_path IS NOT NULL) GROUP BY rrce.userid HAVING SUM(rrc.points) >= 70";
		$remoteready = $db->fetchAll($sql);
		$remotereadyIds = array();
		foreach($remoteready as $userid){
			if (!in_array($userid["userid"], $remotereadyIds)){
				$remotereadyIds[] = $userid["userid"];			
			}
		}
		if (!empty($remotereadyIds)){
			$remoteReadyWhere = " and pe.userid NOT IN (".implode(",", $remotereadyIds).") ";	
		}else{
			$remoteReadyWhere = "";
		}	
	}else{
	...
	/*
	 * Injected By Josef Balisalisa
	 * ->where('userid NOT IN (SELECT userid FROM inactive_staff WHERE is_processed = 0)') //NOT IN INACTIVE
		->where('userid NOT IN (SELECT userid FROM job_sub_category_applicants)') //NOT IN CATEGORIZED
		->where('userid NOT IN (SELECT userid FROM pre_screened_staff WHERE is_processed = 0)') //NOT IN PRESCREENED
		->where('userid NOT IN (SELECT userid FROM remote_ready_criteria_entry_sum_points WHERE is_processed = 0)')
	 */ 
	
	$remoteReadyWhere = " 
		AND us.userid NOT IN (SELECT inactive_staff.userid FROM inactive_staff WHERE is_processed = 0)
		AND us.userid NOT IN (SELECT userid FROM job_sub_category_applicants)
		AND us.userid NOT IN (SELECT userid FROM pre_screened_staff WHERE is_processed = 0)
		AND us.userid NOT IN (SELECT userid FROM remote_ready_criteria_entry_sum_points WHERE is_processed = 0 AND points >= 70)
	 ";
	
	
	if (isset($_GET["over_30"])&&$_GET["over_30"]){
		$over30Filter = " and TIMESTAMPDIFF(DAY, pe.datecreated, CURDATE()) > 30";
	}else{
		$over30Filter = "";
	}
	$login_filter = "";
	...
	if( $posting != ''){
		$select_start = 'select SQL_CALC_FOUND_ROWS a.userid,p.id,a.status,c.latest_job_title,ad.admin_fname,ad.admin_lname,
			...
			inner join unprocessed_staff as us on us.userid = pe.userid ';
		...
		$select_start = 'select SQL_CALC_FOUND_ROWS c.latest_job_title,ad.admin_fname,ad.admin_lname,
			...
			from unprocessed_staff us 
		    left join personal as pe FORCE INDEX (PRIMARY) on pe.userid=us.userid 
		    left join personal_user_logins AS pu on pu.userid = pe.userid 
		    left join currentjob as c on pe.userid=c.userid ';
		$selectCountQuery = "SELECT pe.userid from unprocessed_staff us 
		    left join personal as pe FORCE INDEX (PRIMARY) on pe.userid=us.userid 
		    left join personal_user_logins AS pu on pu.userid = pe.userid 
		    left join currentjob as c on pe.userid=c.userid ";
	...
	$select = "
		".$select_start."
		left join recruiter_staff as rs on rs.userid=pe.userid
		...
		".$login_filter.$posting_filter.$availability_filter.$reg_date_filter.$update_date_filter./*Injected by Josef Balisalisa*/$date_progress_tagged_filter.$recruiter_filter.$keyword_filter.$country_filter.$inactiveUserFilter.$region_filter.$city_filter.$remoteReadyWhere.$over30Filter."
...
$smarty->assign("remote_ready_checked", $remote_ready_checked);
$smarty->assign("over_30", $over_30);
