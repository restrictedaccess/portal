...
if ($_REQUEST["id"]){
	$db->update("tb_shortlist_history", array("rejected"=>0, "feedback"=>"", "date_rejected"=>null, "rejected_by"=>0), $db->quoteInto("id = ?", $_REQUEST["id"]));
	//get userid
	...
	if ($shortlist){
		...
		$history_changes = "Candidate's shortlist to <a href='/portal/leads_information.php?id=".$shortlist["lead_id"]."&page_type=popup' target='_blank'>".$lead["fname"]." ".$lead["lname"]."</a> for the position <a href='/portal/Ad.php?id=".$shortlist["posting_id"]."' target='_blank'>{$shortlist["jobposition"]}</a> has been <span style='color:#ff0000'>unrejected</span>.";
		$changeByType = "admin";
