...
if($_SESSION['admin_id']=="")
{
	...
	if(@isset($_GET["stat"]))
	{
		$id = @$_GET["id"];
		$stat = @$_GET["stat"];
		$a = "UPDATE tb_request_for_interview SET status='$stat' WHERE id='$id'";
		$r_e = mysql_query($a);
	}
	...
	{
		$id = @$_GET["client"];
		$query = "SELECT * FROM tb_request_for_interview WHERE leads_id = '$id'";
	...
	$query = "SELECT DISTINCT(leads_id), fname, lname, email
	FROM tb_request_for_interview WHERE status='ARCHIVED'";
	if(@isset($rt))
	{	
		if(@isset($key))
		{
			$query = "SELECT DISTINCT(leads_id), fname, lname, email
			FROM tb_request_for_interview WHERE (DATE_FORMAT(date_added,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (fname LIKE '%$key%' OR lname LIKE '%$key%') AND status='ARCHIVED'";
		}
		else
		{
			$query = "SELECT DISTINCT(leads_id), fname, lname, email
			FROM tb_request_for_interview WHERE (DATE_FORMAT(date_added,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND status='ARCHIVED'";
		}
	}
...
<center>
					<table width="100%" border="0" bgcolor="#F1F1F3" cellpadding="10" cellspacing="1">
                      <tr>
                        <td align="left" valign="top" bgcolor="#ffffff" colspan="5> <font color="#000000"><strong>
                          <form action="" method="post" name="formtable" onSubmit="if (this.report_t.value =='') return false;">
							...
							<td><input type="text" id="key_id" name="key" value="<?php echo $key; ?>" class="select" /></td>							  
                                <td scope="row"><font color="#000000"><strong>Date&nbsp;Requested</strong></font></td>
                                ...
                                <td valign="top"><font color="#000000">
                                  <input type="button" onclick="javascript: window.location='admin_request_for_interview.php'; " value="Go to Active Page" class="button" name="submit">
                                </td>
                              </tr>
                            </table>
                          </form>
                      ...
                      <tr>
                        ...
                        <td align="left" valign="top" bgcolor="#F1F1F3"><strong>Date&nbsp;of&nbsp;Interview</strong></td>
                        <td align="left" valign="top" bgcolor="#F1F1F3"><font color="#000000"><strong>Time</strong></font></td>
						...
						<td align="left" valign="top" bgcolor="#F1F1F3"><font color="#000000"><strong>Client&nbsp;Comment</strong></font></td>
                      </tr>	
...
?>		  
                      <tr>
                        <td align="left" valign="top" bgcolor="#F1F1F3" colspan="12">
								<table>
									<tr>
										<td><a href="?rt=<?php echo $rt; ?>&key=<?php echo $key; ?>&client=<?php echo $id; ?>"><img src='images/back.png' border=0 alt='Move to Archive'></a></td><td><font color="#000000"><strong><?php echo strtoupper($row['fname']." ".$row['lname']); ?></strong></font></td>
									</tr>
								</table>						
						</td>
                      </tr>					  
...
<?php
						...
						$query = "SELECT * FROM tb_request_for_interview WHERE leads_id='$id' AND status='ARCHIVED'";
						if(@isset($rt))
						{	
							if(@isset($key))
							{
								$query = "SELECT *
								FROM tb_request_for_interview 
								WHERE (DATE_FORMAT(date_added,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (leads_id='$id') AND (fname LIKE '%$key%' OR lname LIKE '%$key%') AND status='ARCHIVED'";
							}
							else
							{
								$query = "SELECT *
								FROM tb_request_for_interview WHERE (DATE_FORMAT(date_added,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (leads_id='$id') AND status='ARCHIVED'";
							}
						}
						$r = mysql_query($query);
...
?>
						  <tr>
							<td align="left" valign="top" bgcolor="#FFFFFF">
								<table>
									<tr>
										<td><a href="?key=<?php echo $key; ?>&rt=<?php echo $rt; ?>&status=<?php echo $status; ?>&stat=<?php echo "NEW"; ?>&id=<?php echo $row["id"]; ?>"><img src='images/back.png' border=0 alt='Move to Archive'></a></td>
										<td>
											<a href="javascript: resume(<?php echo $row["userid"]; ?>); ">
											<?php 
												$name = mysql_query("SELECT fname, lname FROM personal WHERE userid='".$row["applicant_id"]."' LIMIT 1");
							...
							<td align="left" valign="top" bgcolor="#FFFFFF"><?php echo @$row["date_interview"]; ?></td>
							<td align="left" valign="top" bgcolor="#FFFFFF"><?php echo @$row["time"]; ?></td>
							<td align="left" valign="top" bgcolor="#FFFFFF"><?php echo @$row["time_zone"]; ?></td>
							<td align="left" valign="top" bgcolor="#FFFFFF"><?php echo @$row["date_added"]; ?></td>
