...
//echo $type;
$testimony_reply_id  = $_REQUEST['testimony_reply_id'];
...
if($status =="delete"){
	$query = "DELETE FROM testimonials_reply WHERE testimony_reply_id = $testimony_reply_id ;";
	$result = mysql_query($query);				
	if(!$result) die ("There was a problem in deleting...<br>".$query);
	echo "<b>Testimony Deleted!!!</b>";
}
if($status !="delete"){
	$query = "UPDATE testimonials_reply SET 
						...
						WHERE testimony_reply_id = $testimony_reply_id ;";

	$result = mysql_query($query);				
	if(!$result) die ("There was a problem in saving your reply");



$sql = "SELECT  created_by_id, created_by_type, DATE_FORMAT(date_created,'%D %b %Y'), testimony_reply , testimony_reply_status , DATE_FORMAT(date_updated,'%D %b %Y') 
			FROM testimonials_reply 
			WHERE testimony_reply_id = $testimony_reply_id
			ORDER BY date_created ASC;";
$data = mysql_query($sql);
list($reply_by_id, $reply_by_type, $reply_date_created, $testimony_reply , $testimony_reply_status , $reply_date_updated)=mysql_fetch_array($data);

$testimony_reply = str_replace("\n","<br>",$testimony_reply) ;
		echo "<div id ='testimony_reply_id_$testimony_reply_id' style='font:11px tahoma;'>
