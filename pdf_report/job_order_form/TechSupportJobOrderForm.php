...
$job_order_form_title = "Tech Support Job Order Form";
$job_order_form_desc = "The main responsibility of a Tech Support is to create and foster goodwill among the clients. A Tech Support Agent shall make outbound calls and receive inbound calls to support existing clients and leads (whichever the client assigns), prepare daily report on calls results. It is also the duty of the tech support to qualify prospect and build relationships with clients through prompt, courteous, and professional service. The candidate for this position is a college graduate of any computing course or any related course, with minimum of 2 years related work experience; must be fluent in English. ";
	...
	<div style="border:#999999 solid 1px; width:900px; float:left; margin-left:100px;">
		<div style=" line-height:20px; background:#CCCCCC; border:#CCCCCC outset 1px;"><b>
			...
			<div style="float:left; width:94px; border-left:#999999 solid 1px; ">
				...
				<div style="float:right;"><input type="button" value="add" style="font:10px tahoma; width:30px;" onclick="show_hide('tech_support_add_form');" />
		...
		<div id="tech_support_add_form" style="border:#000000 solid 1px; padding:3px; display:none;">
			<input type="text" id="tech_support_skill" name="tech_support_skill" class="select" style="width:750px;"  />
			<select id="tech_support_skill_rating" name="tech_support_skill_rating" style="font:12px tahoma; width:40px;"  >
				<option value="">--</option>
			...
			<input type="button" value="save" style="font:11px tahoma; width:40px;" onclick="saveTecSupportSkills(801);" />
			<input type="button" value="cancel" style="font:11px tahoma; width:40px;" onclick="hide('tech_support_add_form');" />
		</div>
		<div id="tech_support">
			<?
			$query = "SELECT job_order_details_id, job_requirement, rating FROM job_order_details j WHERE job_order_id = $job_order_id AND job_order_form_id = $job_order_form_id  AND groupings = 'tech_support';";
				...
				?>
					<div style=" line-height:20px;  border-bottom:#CCCCCC solid 1px;">
						...
						<div style="float:left; width:94px; border-left:#999999 solid 1px; text-align:center;">
							<div style="float:left">&nbsp;&nbsp;<?=$rating;?></div>
							<div style="float:right"><a href="javascript:deleteJobOrderDetails(<?=$job_order_details_id;?>,'tech_support',801,<?=$job_order_form_id;?>);">X</a>&nbsp;&nbsp;</div>
						</div>
						<div style="clear:both;"></div>
					</div>
			<?
