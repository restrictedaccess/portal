...
//echo sprintf('%s => %s', $comment_by_id, $comment_by_type);
//exit;
$date = $_REQUEST['date'];
...
for($i=0; $i<count($dates);$i++){
		//parse the date  of leave
		$sql = $db->select()
			->from('leave_request_dates' , 'date_of_leave')
			->where('id =?' ,$dates[$i]);
		$date_of_leave = $db->fetchOne($sql);
		
		$det = new DateTime($date_of_leave);
...
//insert history
$history_changes = strtoupper($mode)." DATES : <br>".$date_of_leave_str."<br><em>".$notes."</em>";
$data = array(
