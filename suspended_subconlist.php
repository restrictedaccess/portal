...
$page_status = 'SUSPENDED';

if($_SESSION['admin_id']==""){
	header("location:index.php");
	exit;
}
header("location:/portal/django/subcontractors/subcons/suspended");
exit;
...
// get all subcon
$sql = "SELECT DISTINCT(s.userid), p.fname, p.lname , p.email FROM subcontractors s  JOIN personal p  ON p.userid = s.userid WHERE s.status='suspended' ORDER BY p.fname ASC;";
...
$sql = "SELECT COUNT(id)AS numrows FROM subcontractors s  WHERE s.status='suspended';";
...
(s.status)AS contract_status, s.client_finish_work_hour, l.business_partner_id, rs.admin_id, l.hiring_coordinator_id, s.service_type
...
$row_results = $numrows." record/s found.";
...
$sql = "SELECT COUNT(s.id)AS no_staff, s.leads_id , l.fname, l.lname FROM subcontractors s JOIN leads l ON l.id = s.leads_id WHERE s.status='suspended' GROUP BY s.leads_id ORDER BY l.fname;";
