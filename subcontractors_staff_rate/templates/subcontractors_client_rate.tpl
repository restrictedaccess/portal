...
<ul>{$paging}</ul>
<br clear="all" />
<input type="submit" name="submit" value="save client rates" />
</div>

<ol start="{$offset}">
    {foreach from=$SUBCONS name=subcon item=subcon}
    <li style="border-bottom:#CCC solid 1px; margin-bottom:10px;"><input type="checkbox" name="subconid[]" {if $subcon.executed} disabled="disabled" {else} checked="checked" {/if} value="{$subcon.subcon.id}" />
        <a href="../contractForm.php?sid={$subcon.subcon.id}" target="_blank">#{$subcon.subcon.id}</a>
        {$subcon.personal.fname} {$subcon.personal.lname} 
        <ol>{if $subcon.executed}<strong>Successfully Recorded Client Rates</strong>{else}Client Price Updates History{/if}
            {foreach from=$subcon.client_rates name=client_rate item=client_rate}
                <li>{$client_rate.start_date|date_format} - {$client_rate.work_status} {$client_rate.rate}</li>  
                {foreachelse}
                    None    
            {/foreach}
        </ol>
