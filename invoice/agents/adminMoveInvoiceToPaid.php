<?php
    ...
    $query = "UPDATE agent_invoice SET status='posted', post_date='$now_str' where id = $invoice_id";
    $rows_affected = $dbh->exec($query);
    if ($rows_affected == 1) {
        echo "<h3>Succesfully moved invoice number $invoice_id to the Paid section.</h3>";
