<?php
    ...
    $query = "UPDATE agent_invoice SET status='deleted', updated_by=$admin_id, updated_by_type='admin' where id = $invoice_id";
    $rows_affected = $dbh->exec($query);
    if ($rows_affected == 1) {
        echo "<h3>Succesfully deleted invoice number $invoice_id</h3>";
        exit;
    }
