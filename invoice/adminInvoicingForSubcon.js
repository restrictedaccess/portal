...
//  Removed codes related to "Full Payment Details"
//2010-01-14 Lawrence Sunglao <lawrence.sunglao@remotestaff.com.au>
//  Removed the old method of generating staff invoice
//2009-10-28 Lawrence Sunglao <lawrence.sunglao@remotestaff.com.au>
//  Added Launcher for mass generation of staff invoice
//2009-08-28 Lawrence Sunglao <lawrence.sunglao@remotestaff.com.au>
//  Added CSV export for drafts and approved
//2009-08-24 Lawrence Sunglao <lawrence.sunglao@remotestaff.com.au>
//  Added CSV export
...
function ClassInvoiceMainFunctions(id) {
    ...
    function OnClickCreateFromTimeRecord(e) {
        window.open("/portal/invoice/GenerateStaffInvoice/GenerateStaffInvoice.html");
    }

    function OnClickCreateBlankInvoice(e) {
        replaceChildNodes('invoice_right_pane', 'Loading...');
        d_new_invoice = doSimpleXMLHttpRequest(INVOICE_PATH + 'adminGetNewInvoiceForm.php', {'mode': 'blank'});
    ...
    function OnSuccessGetNewInvoiceFormBlank(e) {
        ...
        if ($('select_subcontractor') != null) {
            connect('select_subcontractor', 'onchange', OnChangeSubcontractor);
        }
    }
    ...
    function OnClickSubmitCreateInvoice(e) {
        ...
        var userid = $('select_subcontractor').value;
        var invoice_date = $('invoice_date').value;
        var description = $('description').value;
        var mode = 'time_records';
        var query = queryString({'start_date': start_date, 'end_date': end_date, 'invoice_date': invoice_date, 'userid': userid, 'description': description, 'mode': mode});
    ...
    function OnClickSubmitBlankInvoice(e) {
        ...
        var description = $('description').value;
        var mode = 'blank';
        var query = queryString({'invoice_date': invoice_date, 'userid': userid, 'description': description, 'mode': mode});
        d_submit_blank = doXHR(INVOICE_PATH + 'adminCreateInvoice.php', {method:'POST', sendContent: query, headers: {"Content-Type":"application/x-www-form-urlencoded"}});
        d_submit_blank.addCallbacks(OnSuccessCreateInvoice, OnFailCreateInvoice);
    }
...
function ClassInvoiceList(id) {
    ...
    replaceChildNodes(this.id, [
        DIV({'id': 'date-filter'}, [
            ...
            HR(),
            H4(null, "EXPORT TO CSV"),
            DIV(null, [
                BUTTON({'id': this.id + 'btn_export_csv_drafts'}, 'Drafts'),
                BUTTON({'id': this.id + 'btn_export_csv_approved'}, 'Approved'),
                BUTTON({'id': this.id + 'btn_export_csv_paid'}, 'Paid'),
            ]),
            DIV({'class': 'clear'}, null),
        ]),
	...
	connect(this.id + 'btn_export_csv_drafts', 'onclick', this.OnClickExportCSVDrafts);
	connect(this.id + 'btn_export_csv_approved', 'onclick', this.OnClickExportCSVApproved);
	connect(this.id + 'btn_export_csv_paid', 'onclick', this.OnClickExportCSVPaid);
}

ClassInvoiceList.prototype.OnClickExportCSVDrafts = function(e) {
    var start_date = $(this.id + 'start_date').value;
    var end_date = $(this.id + 'end_date').value;
    if (confirm("Are you sure you want to export DRAFT invoices dated " + start_date + " to " + end_date + " to CSV?")) {
        location.href ="invoice/adminExportStaffInvoiceToCSV.php?start_date="+start_date+"&end_date="+end_date+"&status=draft";
    }
}

ClassInvoiceList.prototype.OnClickExportCSVApproved = function(e) {
    var start_date = $(this.id + 'start_date').value;
    var end_date = $(this.id + 'end_date').value;
    if (confirm("Are you sure you want to export APPROVED invoices dated " + start_date + " to " + end_date + " to CSV?")) {
        location.href ="invoice/adminExportStaffInvoiceToCSV.php?start_date="+start_date+"&end_date="+end_date+"&status=approved";
    }
}

ClassInvoiceList.prototype.OnClickExportCSVPaid = function(e) {
    var start_date = $(this.id + 'start_date').value;
    var end_date = $(this.id + 'end_date').value;
    if (confirm("Are you sure you want to export PAID invoices dated " + start_date + " to " + end_date + " to CSV?")) {
        location.href ="invoice/adminExportStaffInvoiceToCSV.php?start_date="+start_date+"&end_date="+end_date+"&status=posted";
    }
}
