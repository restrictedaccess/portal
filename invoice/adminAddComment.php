...
//upon successfull insertion of comment,  return the comments
    ...
    //send email
    //grab subcons email address
    $query = $db->select()
        ->from(array('i' => 'subcon_invoice'), array())
        ->join(array('p' => 'personal'), 
            'i.userid = p.userid',
            array('p.email'))
        ->where('i.id = ?', $invoice_id);

    $subcon_email = $db->fetchOne($query);

    $admin_name = GetName($admin_id, 'admin');

    $now = new DateTime();
    $now_date_time_str = $now->format("Y-m-d H:i:s");

    $mail = new Zend_Mail();
    $mail->setBodyText("Noted By: $admin_name\nNoted On: $now_date_time_str\nInvoice ID: $invoice_id\n\n$comment");
    ...
    $mail->setSubject("Invoice Note by $admin_name on Invoice # $invoice_id");
