...
while(list($id,$leads,$by, $by_type,$quote_no,$status)=mysql_fetch_array($result))
{
	$counter++;
?>
...
//$counter=0;
while(list($id,$leads,$posted_date,$by, $by_type,$quote_no,$status)=mysql_fetch_array($data))
{
	$counter++;
?>
	<div style="padding:5px; border-bottom:#CCCCCC solid 1px;" onmouseover="highlight(this);" onmouseout="unhighlight(this);" onclick="showQuote(<?php echo $id;?>);">
...
$result=mysql_query($sqlServiceAgreement);
$counter = 0;
while(list($service_agreement_id, $by, $by_type, $status,$leads,$quote_no)=mysql_fetch_array($result))
{
	$counter++;
?>
