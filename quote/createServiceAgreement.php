...
list($leads_id)=mysql_fetch_array($data);
...
$service_agreement_id = mysql_insert_id();
...
while(list($work_status, $work_position, $working_hours, $days, $currency, $quoted_price , $gst )=mysql_fetch_array($data))
{
  		$counter++;
		if($currency == "AUD"){
			$currency_symbol = "\$ ";
		...
		$service_agreement_details = getWorkStatusLongDescription($work_status)." ".$work_position." , working ".$working_hours. " hours a day , ".$days." days a week . Monthly Fees: " .$currency.$currency_symbol.number_format($quoted_price,2,'.',',') .$tax_str." + Temporary Currency Fluctuation Fee ." ;
