...
$show_control_buttons=$_REQUEST['show_control_buttons']; 
...
$sql = "SELECT q.id, q.quote_no, date_quoted,l.fname,l.lname, l.email,l.company_name, q.created_by, q.created_by_type , q.date_posted , l.company_address , q.status , ran, q.leads_id FROM quote q LEFT JOIN leads l ON l.id = q.leads_id WHERE q.id = $id;";
...
foreach($quote_details as $d){
	...
	if($d['userid']){
		...
		//$d['candidate'] = sprintf('%s %s', strtolower($personal['fname']), strtolower($personal['lname']));	
		$d['candidate'] = strtolower($personal['fname']);
	}
...
$smarty->assign('show_control_buttons', $show_control_buttons);
$smarty->assign('details', $details);
$smarty->assign('quote', $quote);

$smarty->display('show_quote_details.tpl');	
?>