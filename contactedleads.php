...
for($i=0; $i<count($AffArray);$i++)
{
		if($AffArray[$i]==$agent_no) {
			$sqlCount="SELECT COUNT(id) AS numrows1 FROM leads l  WHERE l.status = 'Contacted Lead' AND agent_id = ".$AffArray[$i];
			//echo $sqlCount;
			$rs2=mysql_query($sqlCount);
			$rows=mysql_fetch_array($rs2);
			$BP_leads =	$rows['numrows1'];
		}else{
			$sqlCount3="SELECT COUNT(id) AS numrows2 FROM leads l  WHERE l.status = 'Contacted Lead' AND agent_id = ".$AffArray[$i];
			//echo $sqlCount;
...
<html>
<head>
<title>Business Partner Contacted Lead</title>
...
<p><label ><img src='images/action_check.gif' title='Your Regstered New Leads' /></label> : Your own Contacted Leads <span style="margin-left:10px;"><b><?=$BP_leads;?></b></span></p>
<div style="clear:both;"></div>
<p ><label><img src='images/adduser16.png' title='Affiliate : $agent_fname&nbsp;$agent_lname New Leads' /></label> : Affiliates Contacted Leads <span style="margin-left:20px;"><b><?=$AFF_leads;?></b></span></p>
<div style="clear:both;">&nbsp;</div>
...
<form name="form" method="post" action="contactedleads.php" >
...
FROM leads l
LEFT OUTER JOIN agent a ON a.agent_no = l.agent_id
WHERE l.status = 'Contacted Lead' AND agent_id = ".$AffiliatesArray[$i]." ORDER BY timestamp DESC;";
}
...
?>
  <tr bgcolor=<? echo $bgcolor;?> class="leads_label">
	...
	echo "<img src='images/adduser16.png' title='$agent_fname&nbsp;$agent_lname Contacted Lead' />";
	}else{
	echo "<img src='images/action_check.gif' title='Your own Regstered Contacted Lead' />";
	}
	?></td>
...
?>
  <tr>
  <td colspan="9"></td>
  <td colspan="2" class="leads_label">
