...
{section name=i loop=$notes}
    ...
    {if $staff_name neq $new_staff_name}
        ...
        <tr bgcolor="#EEEEEE" style="font-weight:bold;"><td width="40px">Date</td><td width="68px">Time</td><td width="418px">Activity Notes</td><td width="54px">Rating</td></tr>
    ...
    {/if}
        <tr bgcolor="{cycle values="#CCFFCC,#EEFFEE"}">
            ...
            <td width="418px" class="notes">- {$notes[i].note|ss}</td>
            <td width="54px">
                {if $notes[i].rate eq 'Poor'}
                    {assign var='stars' value=1}
                {/if}
                {if $notes[i].rate eq 'Nothing special'}
                    {assign var='stars' value=2}
                {/if}
                {if $notes[i].rate eq 'Good'}
                    {assign var='stars' value=3}
                {/if}
                {if $notes[i].rate eq 'Pretty Cool'}
                    {assign var='stars' value=4}
                {/if}
                {if $notes[i].rate eq 'Awesome!'}
                    {assign var='stars' value=5}
                {/if}
                {section name=star loop=$stars}
                    <img src="cid:{$cid_star}" height="10" width="10" style="border-style: none;">
                {/section}
            </td>
        </tr>
