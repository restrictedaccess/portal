...
if($delete=="TRUE")
{
	$sqlDELETE="DELETE FROM admin_notes WHERE id =$id";
	mysql_query($sqlDELETE);
}


//SELECT * FROM admin_notes a;
//id, admin_id, leads_id, starting_date, position, subcon, subcon_no, working_start_date, notes, date_created
...
{	// table : notes
	...
	if($id==""){
	$sql="INSERT INTO admin_notes SET admin_id = $admin_id, 
						leads_id = $leads_id, 
	...
	{
		echo("<html><head><script>function update(){top.location='admin_addNotes.php?leads_id=$leads_id&mess=$mess';}var refresh=setInterval('update()',1200);
...
<form method="post" name="form" action="admin_addNotes.php">
...
<img src="images/space.gif" height=8 width=1><br clear=all>
	...
	<p align="right"><b><a href='admin_addNotes.php?leads_id=<?=$leads_id;?>&control=show'>Add</a> </b></p>
<? if($control=="show") {?>
...
FROM admin_notes n  WHERE n.leads_id = $leads_id;";
//echo $query;
$resulta=mysql_query($query);
$ctr=@mysql_num_rows($resulta);
if ($ctr >0 )
{
	...
	{ //$date_created
		...
		echo "<td colspan=3 align =right>$date_created<br>
				<a href='admin_addNotes.php?leads_id=$leads_id&id=$id&control=show'>Edit</a> | <a href='admin_addNotes.php?leads_id=$leads_id&id=$id&delete=TRUE'>Delete</a></td>";
		echo "</tr>";
