...
//2010-08-09    Lawrence Oliver Sunglao <lawrence.sunglao@remotestaff.com.au>
//  -   major re-write
...
if ($_POST) {
    ...
    echo "<script>alert('Current job updated!');window.close();</script>";
    die();
}
else {
    $userid=$_GET["userid"];
...
//get applicant name
$sql = $db->select()
        ->from('personal', Array('userid', 'lname', 'fname'))
        ->where('userid = ?', $userid);
$applicant_data = $db->fetchRow($sql);
...
$smarty->assign('applicant_data', $applicant_data);

$smarty->display('admin_updatecurrentjob.tpl');
