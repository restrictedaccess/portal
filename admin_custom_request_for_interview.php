...
function search($payment_status,$date_requested1,$date_requested2,$key,$status,$p,$maxp,$max) 
{
	...
	//default	
	$where = "WHERE $date_query (r.leads_id=l.id) AND (r.voucher_number=v.code_number) AND r.status='NEW' AND service_type='CUSTOM'";
	...
	{	
		if($key == "")
		{
			...
			$where = "WHERE $date_query (r.leads_id=l.id) AND r.status='$status' AND (r.voucher_number=v.code_number) AND service_type='CUSTOM'";
			$query = "SELECT $fields $from $where $order_by";
			//ended
		}
		else
		{
			//search w/ keyword
			$where = "WHERE $date_query (r.leads_id=l.id) AND r.status='$status' AND (r.voucher_number=v.code_number) AND (r.voucher_number='$key' OR l.fname LIKE '%$key%' OR l.lname LIKE '%$key%') AND service_type='CUSTOM'";
			$query = "SELECT $fields $from $where $order_by";
			//ended
		}
	}
