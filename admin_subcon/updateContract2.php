...
$data = array (
		...
		'prepaid' => $prepaid,
		'prepaid_start_date' => $prepaid_start_date,
...
$sql = "SELECT * FROM subcontractors_temp WHERE temp_status != 'deleted' AND  id =  $id;";
$result = $db->fetchAll($sql);
if(count($result)>0){
	$status =  1;
}
...
if(! TEST){
	...
	$fullname = "Rica J.";
	$mail->setSubject($site." STAFF ".$staff_name." CONTRACT HAS BEEN EDITED AND UPDATED");

}else{
	$to = 'devs@remotestaff.com.au'; //replace devs@remotestaff.com.au
	$fullname = "Remotestaff Developers";
	$mail->setSubject("TEST".$site." STAFF ".$staff_name." CONTRACT HAS BEEN EDITED AND UPDATED");
