...
$sql = "SELECT subcontractors_id , temp_status FROM subcontractors_temp WHERE id = $id;";
...
$history_changes = compareData($data , $table , $subcontractors_id);
...
$changes = "Updates has been cancelled.<br>";

$changes .= "<b>Changes made : </b>.".$email_history_changes.$skype_history_changes.$email_pass_history_changes.$skype_pass_history_changes."<br>".$history_changes;

$details =  "<h3>STAFF CONTRACT UPDATES HAS BEEN CANCELLED</h3>
...
if($status == 1){
	...
	//SEND MAIL
	if(TEST){
		$mail->setSubject("TEST STAFF ".$staff_name." NEW CREATED CONTRACT HAS BEEN CANCELLED");
...
if($status == 2){
	...
	//SEND MAIL
	if(TEST){
		$mail->setSubject("TEST STAFF ".$staff_name." CONTRACT UPDATES HAS BEEN CANCELLED");
		$mail->addTo('devs@remotestaff.com.au', 'devs');
	    $mail->send($transport);
	}
