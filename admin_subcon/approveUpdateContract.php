...
//get the subcontractors_id
$sql = "SELECT subcontractors_id FROM subcontractors_temp WHERE id = $id AND temp_status = 'updated';";
$result = $db->fetchRow($sql);
...
//Process to approve updated contract
// 1. check if the data are changed or updated
// 2. insert a record to the subcontract_history
...
$history_changes = compareData($data , "subcontractors" , $subcontractors_id);
...
$changes = "Contract has been updated and approved.<br>";
$changes .= "<b>Changes made : </b>.".$email_history_changes.$registered_email_history_changes.$skype_history_changes.$email_pass_history_changes.$skype_pass_history_changes."<br>".$history_changes;
$data = array (
...
$where = "subcontractors_id = ".$subcontractors_id." AND temp_status = 'updated'";	
...
$details =  "<h3>STAFF CONTRACT UPDATES HAS BEEN UPDATED AND APPROVED</h3>
...
if(! TEST){
	//admin email
	$mail->setSubject(" STAFF ".$staff_name." CONTRACT UPDATES HAS BEEN UPDATED AND APPROVED");
	$mail->addTo('devs@remotestaff.com.au', 'devs');
    $mail->send($transport);
}else{
	$mail->setSubject("TEST STAFF ".$staff_name." CONTRACT UPDATES HAS BEEN UPDATED AND APPROVED");
	$mail->addTo('devs@remotestaff.com.au', 'devs');
    $mail->send($transport);
}
*/
...
$html = $details;
$subject = sprintf('STAFF %s MODIFIED CONTRACT HAS BEEN APPROVED', $staff_name);
$text = NULL;
