...
$happened = false;
//$db->delete("remote_ready_criteria_entries", "userid = ".$userid." AND remote_ready_criteria_id = 4");
$counter = 0;
for($i=0;$i<count($history_company_name);$i++){
	...
	if ($history_monthto[$i]=="Present"||$history_yearto[$i]=="Present"){
		if (!$happened){
			$smarty = new Smarty();
	...
	if (!(trim($history_company_name[$i])==""||$history_position[$i]==""||$history_yearfrom[$i]==""||$history_yearto[$i]==""||$history_monthfrom[$i]==""||$history_monthto[$i]==""||$history_position[$i]==""||$history_responsibilities[$i]=="")){
		if ($counter!=5){
			$data = array();
			$data["userid"] = $userid;
