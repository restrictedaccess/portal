...
if($userid!="" or $userid!=NULL){
	...
	//$voice = '/portal/'.$result['voice_path'];
	$voice = $result['voice_path'];
	

	//if($voice=="")$image="/portal/images/uploadrecording.jpg";
...
if($userid) {
	$sql=$db->select()
		->from('personal', array('email'))
		->where('userid = ?' ,$userid);  
	$emailaddr = $db->fetchOne($sql);
}

//right menus
$right_menus = RightMenus(7 , $userid);
...
$smarty->display('registernow-step7-upload-voice-recording.tpl');
