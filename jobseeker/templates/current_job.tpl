...
<html lang="en">
	<head>
		...
		<title>Update your Work Experience and Position Desired Details - Remotestaff</title>
		...
		<link rel="stylesheet" href="/portal/jobseeker/css/current_job.css"/>
		...
		<script type="text/javascript" src="/portal/jobseeker/js/update_current_job.js"></script>
	</head>
			...
			<div class="row-fluid" id="main-content">
				...
				<div class="span9" id="main-content-body">
					<div class="container">
						...
						<h2 class="jobseeker-header">Update your Work Experience and Position Desired Details</h2>
						<p class="label label-info">
							Fill in this section to give employers a snapshot of your profile.
						</p>
						<form class="well form-horizontal" method="POST" id="current-job-form" action="/portal/jobseeker/update_work_experience.php">
							{if $success eq "true"}
							<div class="alert alert-success">
								<strong>Well done!</strong> You have successfully updated your work experience and position desired details.
							</div>
							{/if}
							<legend>
								Work Experience and Position Desired
							</legend>
							...
							<div class="control-group">
								...
								<div class="controls">
									<label class="radio">
										<input type="radio" name="freshgrad" value="2" id="freshgrad_2" { if $currentjob.freshgrad eq 2}checked="checked"{ /if }/>
										I am still pursuing my studies and seeking internship or part-time jobs </label>
									<label class="radio">
										<input type="radio" name="freshgrad" value="1" id="freshgrad_1" { if $currentjob.freshgrad eq 1}checked="checked"{ /if }/>
										I am a fresh graduate seeking my first job </label>
									<label class="radio inline">
										<input type="radio" name="freshgrad" value="0" id="freshgrad_0" { if $currentjob.freshgrad eq 0}checked="checked"{ /if }/>
										I have been working for
									</label>
									
										<select name="years_worked" class="span2" style="margin-bottom:5px">
											{section name=years loop=17}
											{if $smarty.section.years.index eq $currentjob.years_worked }
											<option value="{ $smarty.section.years.index }" selected="selected"> { $smarty.section.years.index } </option>
											{else}
										...
										<select name="months_worked" class="span2">
											...
											selected="selected"
											{ /if }> {$smarty.section.months.index} </option>
											{/section}
										</select> month(s)
								</div>
								...
								<p>Fill in your most recent employment history depending on the number of employment history. (optional but recommended)</p>
								...
								<div class="control-group">
									...
									<div class="controls">
										...
										<select name="{ $yearfrom }" class="span2">
											...
											selected="selected"
											{ /if }
											> { $smarty.section.year.index } </option>
											{ /section }
										</select>
								...
								<div class="control-group">
									...
									<div class="controls">
										<label class="radio inline">
											...
											{ /if }
											/>
											I can start work after</label>
											{ strip }
											<select name="available_notice" class="span2"  style="margin-bottom:5px">
												<option value=""></option>
												...
												selected="selected"
												{ /if }
												> { $smarty.section.available_notice.index } </option>
												{ /section }
											</select> { /strip } week(s) of notice period<br/>
										<label class="radio inline">
											...
											/>
											I can start work after </label>
											<select name="aday" class="span2" style="margin-bottom:5px">
												<option value=""></option>
												...
												selected="selected"
												{ /if }
												> { $smarty.section.aday.iteration } </option>
												{ /section }
											</select> -
											<select name="amonth" class="span2">
												...
												selected="selected"
												{ /if }
												> { $month_date|date_format:"%B" } </option>
												{ /section }
											</select> -
										...
										<label class="radio">
											...
											{ /if }
											/>
											I am not actively looking for a job now </label>
										<label class="radio">
								...
								<div class="control-group">
									...
									<div class="controls">
										...
										<select name="salary_currency" class="span3">
											<option value=""></option>
								...
								<div id="character_references">
									...
									<div class="character_reference">
										...
										<div class="control-group">
											...
											<div class="controls">
												<input type="text" class="span6 name" value="{$character_reference.name}" name="name[]"/>
											</div>
										</div>
										<div class="control-group">
											<label class="control-label">Contact Details</label>
											<div class="controls">
												<textarea name="contact_details[]" rows="8" class="span12 contact_details">{$character_reference.contact_details}</textarea>
											</div>
										</div>
										<div class="control-group">
											<label class="control-label">Contact Number</label>
											<div class="controls">
												<input type="text" class="span6 contact_number" value="{$character_reference.contact_number}" name="contact_number[]"/>
											</div>
										</div>
										<div class="control-group" style="margin-bottom:3em">
											<label class="control-label">Email Address</label>
											<div class="controls">
												<input type="text" class="span6 character_email_address email_address" value="{$character_reference.email_address}" name="email_address[]"/>
											</div>
										</div>
									</div>
							...
							<button class="btn btn-primary" id="save_changes_button">
								Update Work Experience and Position Desired
							</button>
						</form>
					</div>
				</div>
