...
function search($rt,$key,$status,$p,$maxp,$max) 
{
	...
	//default	
	$where = "WHERE (DATE_FORMAT(e.date,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (e.leads_id=l.id) AND (e.applicant_id=p.userid) AND e.status='ACTIVE'";
	...
	{	
		if($key == "")
		{
			...
			$where = "WHERE (DATE_FORMAT(e.date,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (e.leads_id=l.id) AND (e.applicant_id=p.userid) AND e.status='$status' ";
			$query = "SELECT $fields $from $where $order_by";
			//ended
		}
		else
		{
			//search w/ keyword
			$where = "WHERE (DATE_FORMAT(e.date,'%Y-%m-%d') BETWEEN '$a_' AND '$b_') AND (e.leads_id=l.id) AND (e.applicant_id=p.userid) AND e.status='$status' AND ((e.email='$key') OR (p.fname LIKE '%$key%' OR p.lname LIKE '%$key%') OR (l.fname LIKE '%$key%' OR l.lname LIKE '%$key%'))";
			$query = "SELECT $fields $from $where $order_by";
			//ended
		}
	}
