...
<div align="left">
    <div id="lr_details">
    	<p><label>Leave Request ID</label> : <strong>{$leave_request.id}</strong></p>
        ...
        <p><label>Contract Id</label> : <a href="/portal/contractForm.php?sid={$subcon.id}" target="_blank">{$subcon.id}</a> <span class="label label-info">{$subcon.status}</span></p>
        ...
        <p><label>Date Requested</label> : {$leave_request.date_requested|date_format}</p>
    </div>
    <hr />
    <div><button id="checkall" class="btn">check all</button></div>
    {if $dates}
