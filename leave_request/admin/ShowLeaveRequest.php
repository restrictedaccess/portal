...
$sql="SELECT * FROM leave_request l where id={$leave_request_id}";
$leave_request = $db->fetchRow($sql);

$sql="SELECT * FROM leave_request_dates l where leave_request_id={$leave_request_id}";
...
$sql="SELECT id, job_designation, status FROM subcontractors WHERE leads_id=".$leave_request['leads_id']." AND userid=".$leave_request['userid']." AND status IN('ACTIVE', 'suspended', 'resigned', 'terminated');";
