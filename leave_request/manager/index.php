...
if($manager['view_staff'] == 'specific'){
	...
	$search_str2=sprintf(' AND s.userid IN(%s)', $staff_search_str);
...
$sql="SELECT DISTINCT(d.date_of_leave) FROM leave_request_dates d JOIN leave_request l ON l.id = d.leave_request_id WHERE l.leads_id=".$_SESSION['client_id'].$staff_search_str." AND YEAR(d.date_of_leave)='".$yearID."' ".$search_str.";";
//echo $sql;exit;
...
foreach($leave_request_dates as $leave_request_date){
	...
	$sql="SELECT COUNT(d.id)AS pending_num FROM leave_request_dates d JOIN leave_request l ON l.id = d.leave_request_id WHERE l.leads_id=".$_SESSION['client_id'].$staff_search_str." AND d.date_of_leave='".$leave_request_date['date_of_leave']."' AND d.status='pending' ".$search_str." GROUP BY d.status;";
	//echo $sql;exit;
	$pending_num = $db->fetchOne($sql);
	$sql="SELECT COUNT(d.id)AS approved_num FROM leave_request_dates d JOIN leave_request l ON l.id = d.leave_request_id WHERE l.leads_id=".$_SESSION['client_id'].$staff_search_str." AND d.date_of_leave='".$leave_request_date['date_of_leave']."' AND d.status='approved' ".$search_str." GROUP BY d.status;";
	$approved_num = $db->fetchOne($sql);

	if($pending_num == 0 and $approved_num==0){
	
	    
	$sql="SELECT * FROM leave_request_dates d JOIN leave_request l ON l.id = d.leave_request_id WHERE l.leads_id=".$_SESSION['client_id'].$staff_search_str." AND d.date_of_leave='".$leave_request_date['date_of_leave']."' AND d.status not in ('pending', 'approved') ".$search_str.";";
...
$sql = "SELECT DISTINCT(p.userid) , p.fname , p.lname, s.id FROM personal p JOIN subcontractors s ON s.userid = p.userid WHERE s.status IN('ACTIVE', 'suspended') AND s.leads_id=".$_SESSION['client_id'].$search_str2." ORDER BY p.fname";
//echo $sql;exit;
