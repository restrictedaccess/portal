...
$query = "SELECT l.* FROM clients c JOIN leads l ON l.id = c.leads_id WHERE $status_Search $search_options  ORDER BY timestamp $order_by $limit ";
//echo $query."<br>";//exit;
			...
			*/
			...
			if($lead['marked'] != 'yes' and $lead['ask_question'] != 'yes' and $lead_no_of_orders ==0 ){
				if(!CheckLeadsStaff($lead['id'])){
					...
					$leads_list .= "</td>";
					$leads_list .= "<td valign='top' width='45%' class='identical'>".LeadsOf($agent_affiliate_id, $business_partner_id)."</td>";
