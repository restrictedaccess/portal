...
<table width="100%" cellspacing="0" >
  ...
  <td style=" padding:5px;">
	<form name='regform' method='POST' action='seat_client_manage.php' style='padding:0px;'>
...
<div id='staff'>
<ul class="tabs">
  <li><a href="#clientactive">{$booking_info[0].cfname} {$booking_info[0].clname}</a></i></li>
</ul>
  ...
  <div id="clientactive" class="pane_content">
	 ...
	 {* staff list*}	 
	  <table cellpadding='3' cellspacing='3' class='list' width='100%'>
		<tr>
		  {if !$seat_id}<td class='header'>Seat#</td>{/if}
        ...
        {*if $booking_info|@count > 0*}
          ...
          <tr bgcolor="{cycle values='#d0d8e8,#e9edf4'}" id="book{$booking_info[idx].id}">
			{if !$seat_id}<td class='item'>{$booking_info[idx].seat_id}</td>{/if}
			<td class='item'>{$booking_info[idx].book_date}</td>
			<td class='item'>{$booking_info[idx].book_start} to {$booking_info[idx].book_end}</td>
			
			<td class='item'><a href='seat_staff_manage.php?task=staff_booking&staff_id={$booking_info[idx].userid}'>{$booking_info[idx].fname} {$booking_info[idx].lname}</td>
			...
			<a href="javascript:editBooking('{$booking_info[idx].id}','{$booking_info[idx].seat_id}','{$booking_info[idx].userid}','{$booking_info[idx].fname}', '{$booking_info[idx].lname}', '{$booking_info[idx].date_ymd}',
...
function editBooking(id, seat, userid, fname, lname, date, starthr, endhr, type, payment, status) {
...
<div>
  ...
  <br>
	...
	<form name='regform' method='POST' target='ajaxframe' action='seat_client_manage.php' style='padding:0px;'>
  ...
  <div style='width:570px;padding:3px;border:1px solid #011a39;'>
	<div style='text-align:center;padding-left:10px;border:none;font-size:13px;font-weight:bold;'>Modify Booking</div>
		<form name='regform' method='POST' target='ajaxframe' action='seat_client_manage.php' style='padding:0px;'>
			...
			<table width="100%" border='0' cellpadding="2" cellspacing="2" class="list">
			  ...
			  <tr><td class='form1' id='client_header'>Client Name: </td><td class='form2'>{$booking_info[0].cfname} {$booking_info[0].clname}</td></tr>
	...
	function createResult(err_msg, is_error) {
		if( is_error )
			...
			location.href='seat_client_manage.php?task={/literal}{$task}{literal}&client_id='+client_id;
		}
	}
	//-->
	</script>{/literal}
