...
if($_SESSION['admin_id']=="")
{
	...
	if(@isset($_GET["stat"]))
	{
		$id = @$_GET["id"];
		$stat = @$_GET["stat"];
		$a = "UPDATE tb_endorsement_history SET status='$stat' WHERE id='$id'";
		$r_e = mysql_query($a);		
	}
	...
	{
		$id = @$_GET["client"];
		$query = "SELECT * FROM tb_endorsement_history WHERE client_name = '$id'";
		$r_e = mysql_query($query);
		while ($row = mysql_fetch_assoc($r_e)) 
		{
			$id = $row["id"];
			$stat = $row["status"]."(archived)";
			mysql_query("UPDATE tb_endorsement_history SET status='$stat' WHERE id='$id'");
		}
	}
	...
	{	
		...
		{
			$stat = "AND e.client_name = '$selected_client' ";		
		}
		switch($status)
		{
			case "On Hold":
				$stat = $stat."AND e.status='On Hold' ";
				break;
			case "Hired":
				$stat = $stat."AND e.status='Hired' ";
				break;
			case "Rejected":
				$stat = $stat."AND e.status='Rejected' ";
				break;
			default:
				$stat = $stat."AND (e.status='On Hold' || e.status='Hired' || e.status='Rejected' || e.status='Active') ";
				break;	
		}
...
<center>
					<table width="100%" border="0" bgcolor="#F1F1F3" cellpadding="10" cellspacing="1">
                        ...
                        <td align="left" valign="top" bgcolor="#ffffff" colspan="6> <font color="#000000"><strong>
                          <form action="" method="post" name="formtable" onSubmit="if (this.report_t.value =='') return false;">
							...
							<td>	  
                                ...
                                <td valign="top"><font color="#000000">
                                  <input type="button" onclick="javascript: window.location='application_endorsement_summary_archive.php'; " value="Go to Archived Page" name="submit">
                                </td>
                              </tr>
                            </table>
                          </form>
					  ...
					  <tr><td width=100% colspan=6>Showing Result for: <?php echo @$title; ?></td></tr>
                      ...
                      <tr>
						...
						<td align="left" valign="top" bgcolor="#F1F1F3" width=100%><font color="#000000"><strong>Comment</strong></font></td>
                      </tr>	
...
?>		  
                      <tr>
                        <td align="left" valign="top" bgcolor="#F1F1F3" colspan="12">
								<table>
									<tr>
										<td><a href="?rt=<?php echo $rt; ?>&status=<?php echo $status; ?>&client=<?php echo $row["client_name"]; ?>"><img src='images/forward.png' border=0 alt='Move to Archive'></a></td><td><font color="#000000"><strong><?php echo strtoupper($row['fname']." ".$row['lname']); ?></strong></font></td>
									</tr>
								</table>						
						</td>
                      </tr>					  
...
<?php
						...
						{
							case "On Hold":
								$stat = "AND status='On Hold'";
								break;
							case "Hired":
								$stat = "AND status='Hired'";
								break;
							case "Rejected":
								$stat = "AND status='Rejected'";
								break;
							default:
								$stat = "AND (status='On Hold' || status='Hired' || status='Rejected' || status='Active')";
								break;	
						}
...
?>
						  <tr>
							<td align="left" valign="top" bgcolor="#FFFFFF">
								<table>
									<tr>
										<td><a href="?rt=<?php echo $rt; ?>&status=<?php echo $status; ?>&stat=<?php echo $row["status"]."(archived)"; ?>&id=<?php echo $row["id"]; ?>"><img src='images/forward.png' border=0 alt='Move to Archive'></a></td><td><?php echo @$row["position"]; ?><em>(<a href="javascript: update(<?php echo $row["id"]; ?>); ">update</a>)</em></td>
									</tr>
								</table>
