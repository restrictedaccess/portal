<?php
$leads_list .= "<tr bgcolor='#FFFF00'>";
$leads_list .= "<td width='80%' colspan='6' valign='top' style='padding:5px;color:black;'>";
$leads_list .= "<div><b>THE LIST IS IN BUSINESS DEVELOPER GROUPINGS VIEW</b></div>";
$leads_list .= "</td>";
$leads_list .= "</tr>";
...
foreach($agents as $agent){
		...
		$leads_list .= "<td width='20%' align='right' style='color:#FFFFFF;'>";
			$queryCount = "SELECT COUNT(id)AS numrows FROM leads l WHERE $status_Search $search_options AND agent_id = ".$agent['agent_id'];
			$resultCount = $db->fetchRow($queryCount);
			$numrows = $resultCount['numrows'];
			//$leads_list .= $queryCount;
		
			if($search_flag == False){
				$leads_list .= "<select style='float:left;' id='".$agent['agent_id']."_select' onchange='paging(".$agent['agent_id'].")'>";
					// how many pages we have when using paging?
					$self = "./".basename($_SERVER['SCRIPT_FILENAME'])."?agent_id=".$agent['agent_id']."&lead_status=".$lead_status;
					...
					for($page = 1; $page <= $maxPage; $page++)
					{
						if ($page == $pageNum)
						{
							if($agent_id == $agent['agent_id']) {
								...
								$leads_list .= " <option selected value=\"$self&page=$page&rowsPerPage=$rowsPerPage#".$agent['agent_id']."\">Page $page</option> ";
		
							}else{
								$leads_list .= " <option value=\"$self&page=$page&rowsPerPage=$rowsPerPage#".$agent['agent_id']."\">Page $page</option> ";
							}
						}
						else
						{
		...
		$query = "SELECT * FROM leads l WHERE $status_Search $search_options AND agent_id = ".$agent['agent_id']." ORDER BY timestamp $order_by $limit ";
		//echo $query."<br>";;
		...
		foreach($leads as $lead){
			...
			//	$leads_owner = "<span class=\"leads_owner\">Please Update leads information. No Service by Business Partner detected.</span>";
			//}
					...
					$leads_list .= "</td>";
						...
						$leads_list .= "<td width='18%' valign='top'>";	
							...
							$leads_list .= "<div style='color:#000000;padding-left:10px;padding-top:5px;'>";
								...
								//$leads_list .="<div>ASL Orders : ".$lead_no_of_orders."</div>";
							$leads_list .= "</div>";
						$leads_list .= "</td>";	
