...
for($i=0; $i<count($AffArray);$i++)
{
		if($AffArray[$i]==$agent_no) {
			$sqlCount="SELECT COUNT(id) AS numrows1 FROM leads l  WHERE l.status = 'Follow-Up' AND agent_id = ".$AffArray[$i];
			//echo $sqlCount;
			$rs2=mysql_query($sqlCount);
			$rows=mysql_fetch_array($rs2);
			$BP_leads =	$rows['numrows1'];
		}else{
			$sqlCount3="SELECT COUNT(id) AS numrows2 FROM leads l  WHERE l.status = 'Follow-Up' AND agent_id = ".$AffArray[$i];
			//echo $sqlCount;
...
<html>
<head>
<title>Business Partner Follow-Up</title>
...
<ul class="glossymenu">
 ...
 <li class="current"><a href="follow_up_leads.php"><b>Follow Up</b></a></li>
...
<p><label ><img src='../images/action_check.gif' title='Your Regstered Follow-Up' /></label> : Your own Registered Follow-Up <span style="margin-left:10px;"><b><?=$BP_leads;?></b></span></p>
<div style="clear:both;"></div>
<p ><label><img src='../images/adduser16.png' title='Affiliate : $agent_fname&nbsp;$agent_lname Follow-Up' /></label> : Affiliates Follow-Up <span style="margin-left:20px;"><b><?=$AFF_leads;?></b></span></p>
<div style="clear:both;">&nbsp;</div>
...
<form name="form" method="post" action="follow_up_leads.php" >
...
FROM leads l
LEFT OUTER JOIN agent a ON a.agent_no = l.agent_id
WHERE l.status = 'Follow-Up' AND agent_id = ".$AffiliatesArray[$i]." ORDER BY timestamp DESC;";
}
...
?>
  <tr bgcolor=<? echo $bgcolor;?> class="leads_label">
	...
	echo "<img src='../images/adduser16.png' title='$agent_fname&nbsp;$agent_lname Follow-Up' />";
	}else{
	echo "<img src='../images/action_check.gif' title='Your own Regstered Follow-Up' />";
	}
	?></td>
