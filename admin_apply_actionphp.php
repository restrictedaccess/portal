<?php
//from : admin_apply_action.php
/*
2009-10-06 Normaneil Macutay
	- Added zend mail function with attachments
...
if($job_order_form=="with"){ 
	...
	$ran = get_rand_id();
	$ran = CheckRan($ran,'job_order');
	$query = "INSERT INTO job_order SET leads_id = $id, created_by_id = $admin_id, created_by_type = 'admin', date_created = '$ATZ', status = 'new' , ran = '$ran';";
	mysql_query($query);
...
<body  >
<p class='message'>".stripslashes($mess)."</p>
<p class='message'>".$signature_template."</p>
...
With such experience in dealing business in both countries, you can feel confident in furthering your business using a remote team. Try us now. 
 <h2>".$link."</h2>
	...
	$query ="INSERT INTO history (agent_no, leads_id, actions, history, date_created , created_by_type) VALUES ($admin_id, $id, 'EMAIL', '$txt', '$ATZ' , 'admin');";
		//echo $query;
		mysql_query($query);
		$history_id = mysql_insert_id();
			// insert 
			// id, history_id, attachment, attachment_type, date_attach, user_type_id, user_type
			if($quote!=NULL){
...
if($bcc!=""){
	$mail->addBcc($bcc);// Adds a recipient to the mail not visible in the header.
}
...
if(!$mail->send($transport)){
	header("location:admin_apply_action.php?id=$id&code=2&page=$page");
	//echo "Email not Sent";
}
else
{
	//echo "Email Sent";
	header("location:admin_apply_action.php?id=$id&code=1&page=$page");
}
	...
	{
		$txt=filterfield($txt);
	$query ="INSERT INTO history (agent_no, leads_id, actions, history, date_created , created_by_type) VALUES ($admin_id, $id, '$action', '$txt', '$ATZ' , 'admin');";
	//echo "Here ". $query;
		$result=mysql_query($query);
		if (!$result)
		{
