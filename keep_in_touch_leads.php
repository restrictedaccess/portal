...
if($lead_status!=""){
	if($lead_status == "all"){
		...
		$lead_status2 = " l.status != 'Inactive' AND  l.status != 'REMOVED' AND l.business_partner_id = $agent_no ";
	}else{
		$folder = $lead_status; 
		$lead_status = " AND l.status = '$lead_status' ";
		if($folder != "Keep In-Touch"){
			$lead_status2 = " l.status = '$folder' AND l.business_partner_id = $agent_no ";
		}else{
			$lead_status2 = " l.status = '$folder' ";
		}
	}
}else{
	$lead_status = " AND l.status = 'Keep In-Touch' ";
	$lead_status2 = " l.status = 'Keep In-Touch' ";
