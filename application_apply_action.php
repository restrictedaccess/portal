...
// - script to delete record on the top 10 applicant

//redirect to script of roy.
$location = sprintf('recruiter/staff_information.php?%s',$_SERVER['QUERY_STRING']);
...
$q ="SELECT ap.status, ap.date, a.admin_fname, a.admin_lname FROM applicant_status ap, admin a WHERE a.admin_id = ap.admin_id AND ap.personal_id = '$userid' AND a.admin_id=".$_SESSION['admin_id'].";";
...
$asl = mysql_query("SELECT id FROM tb_request_for_interview WHERE applicant_id='$userid'");
$ctr=mysql_num_rows($asl);
if ($ctr >0 )
{
...
{
  if((document.form.star.selectedIndex==null)&&(document.form.img.value==null))
  {
    missinginfo = "";
...
<!-- ROY'S CODE ------------------->
    ...
    {
      asl_interview_counter_obj.open('get', 'asl_interview_counter_report.php?userid='+userid)
    ...
    function asl_interview_counter_loader()
    {
      ...
      {
        document.getElementById('asl_interview_counter_loader').innerHTML = data;
      }
      else
      {
        document.getElementById("asl_interview_counter_loader").innerHTML="<img src='images/ajax-loader.gif'>";
      }
    }    
    function asl_interview_counter_exit()
    {
      document.getElementById("asl_interview_counter_loader").innerHTML="";
    }    
    //ENDED - asl interview counter    
    ...
    {
      app_status_obj.open('get', 'application_apply_actdion_applicant_status.php?admin_id='+admin_id+'&userid='+userid+'&status='+status)
    ...
    function app_status_loader()
    {
      ...
      {
        document.getElementById("app_status_loader").innerHTML="<img src='images/ajax-loader.gif'>";
      }
    }
    //ENDED - applicant status
...
<tr>
  <td width=100% colspan=2>
    <div class="subtab">
          <ul>
        <li id="cp" class="selected"><a href="application_apply_action.php?userid=<?php echo $userid; ?>"><span>Candidate's Profile Manager</span></a></li>
              <li id="csro"><a href="application_apply_action_csro.php?userid=<?php echo $userid; ?>"><span>CSRO Report</span></a></li>
      </ul>
        </div> 
  </td>
</tr>
...
<tr>
  <td width="50%" valign="top">
    ...
    <table cellspacing=1 cellpadding=3 width=100%>
      ...
      <tr>
        <td><input type="submit" name="sound_btn" id="sound_btn" value="upload" class="button"></td>
      </tr>      
    </table>    
  ...
  <td valign="top" width="50%">
    ...
    <table cellspacing=1 cellpadding=3 width=100%>
      <tr>
        <td><font color='#000000'><b>File Type/Size</b> <font color="#666666"><em>(doc, pdf or image format) Upload limit per file is 5 MB</em></font></font></td>
      </tr>    
      <tr>
        <td>
          <select name="file_description">
            <option value="Resume" selected>Resume</option>
            <option value="Mock Call" selected>Mock Calls</option>
    ...
    <table cellspacing=1 cellpadding=3 width=100% >
      ...
      <tr>
        <td>
        <input type="button" class="button"  value="Send Sample(s) to Email" onClick="javascript: send_sample(<?php echo $userid; ?>); "/>
        </td>
      </tr>
    </table>  
...
<!-- START PROFILE -->
    ...
    <!-- image -->
        <table width="100%">
          <tr><td colspan="2"><div id="asl_interview_counter_loader"></div></td></tr>
          <tr>
              <td width="50%">
                        <table width="100%">
                                <tr>
                        ...
                        <?
                                ...
                                <td width="80%" class="subtitle" valign="top">
                                            ...
                                            <table width="100%">
                                            <tr><td><b>Candidate ID</b></td><td>:</td><td><? echo $userid;?></td></tr>
                    ...
                    <table width="100%">
                                            ...
                                            <tr>
                                                <td valign="top"><b>Full Time Rate</b></td><td>:</td><td valign="top">
                                                <SELECT name="pull_time_rate" id="pull_time_rate_id" style="color:#000000; font:arial; font-weight:normal; font-size:11px; padding:0px 1px 2px 2px; border:1px #666666 solid;">            
                                                <?php
                                                    //generate currency dropdown per staff
                                                    $q_currency=mysql_query("SELECT p.id, p.code FROM staff_rate s, products p WHERE s.userid='$userid' AND s.product_id=p.id LIMIT 1");
                                                    $c = 0;
                                                    while ($r_currency = mysql_fetch_assoc($q_currency)) 
                                                    {
                                                        $caption = $r_currency['code'].": ";
                                                        $found_id = $r_currency['id'];
                                                        $c=mysql_query("SELECT DISTINCT(c.code), p.amount FROM product_price_history p, currency_lookup c WHERE p.currency_id=c.id AND p.product_id='$found_id' ORDER BY p.date DESC");
                                                        $usd_code = "";
                            while ($cr = mysql_fetch_assoc($c)) 
                                                        {
                                if($cr['code'] == "USD" && $usd_code == "")
                                {
                                  $caption = $caption."[".$cr['code']." ".$cr['amount']."]  ";
                                  $usd_code = "USD";
                                }
                                else
                                {
                                  if($cr['code'] <> "USD")
                                  {
                                    $caption = $caption."[".$cr['code']." ".$cr['amount']."]  ";
                                                        ...
                                                        echo "<option value='".$r_currency['id']."' selected>".$caption."</option>\n";
                                                        $c++;
                                                    }
                                                    if($c == 0) 
                                                    {
                                                    ...
                                                    $q_currency=mysql_query("SELECT id, code FROM products WHERE code LIKE '%SR-%'");
                                                    while ($r_currency = mysql_fetch_assoc($q_currency)) 
                                                    {
                                                        $caption = $r_currency['code'].": ";
                                                        if($r_currency['id'] != $found_id)
                                                        {
                                                            $c_id = $r_currency['id'];
                                                            $c=mysql_query("SELECT DISTINCT(c.code), p.amount FROM product_price_history p, currency_lookup c WHERE p.currency_id=c.id AND p.product_id='$c_id' ORDER BY p.date DESC");
                                                            $usd_code = "";
                              while ($cr = mysql_fetch_assoc($c)) 
                                                            {
                                if($cr['code'] == "USD" && $usd_code == "")
                                ...
                                else
                                {
                                  if($cr['code'] <> "USD")
                                  {
                                    $caption = $caption."[".$cr['code']." ".$cr['amount']."]  ";
                                                            ...
                                                            echo "<option value='".$r_currency['id']."'>".$caption."</option>\n";
                                                        }  
                                                    }    
                                                    //end            
                                                ?>
                                            ...
                                            <tr>
                                                <td valign="top"><b>Part Time Rate</b></td><td>:</td><td valign="top">
                                                <SELECT name="part_time_rate" id="part_time_rate_id" style="color:#000000; font:arial; font-weight:normal; font-size:11px; padding:0px 1px 2px 2px; border:1px #666666 solid;">            
                                                <?php
                                                    //generate currency dropdown per staff
                                                    $q_currency=mysql_query("SELECT p.id, p.code FROM staff_rate s, products p WHERE s.userid='$userid' AND s.part_time_product_id=p.id LIMIT 1");
                                                    $c = 0;
                                                    while ($r_currency = mysql_fetch_assoc($q_currency)) 
                                                    {
                                                        $caption = $r_currency['code'].": ";
                              ...
                              else
                              {
                                if($cr['code'] <> "USD")
                                {
                                  $caption = $caption."[".$cr['code']." ".$cr['amount']."]  ";
                                                    ...
                                                    $q_currency=mysql_query("SELECT id, code FROM products WHERE code LIKE '%SRPT%'");
                                                    while ($r_currency = mysql_fetch_assoc($q_currency)) 
                                                    {
                                                        $caption = $r_currency['code'].": ";
                                                        if($r_currency['id'] != $found_id)
                                                        {
                                                            $c_id = $r_currency['id'];
                                                            $c=mysql_query("SELECT DISTINCT(c.code), p.amount FROM currency_lookup c, product_price_history p WHERE p.currency_id=c.id AND p.product_id='$c_id' ORDER BY p.date DESC");
                                                            $usd_code = "";
                              while ($cr = mysql_fetch_assoc($c)) 
                                                            {
                                if($cr['code'] == "USD" && $usd_code == "")
                                ...
                                else
                                {
                                  if($cr['code'] <> "USD")
                                  {
                                    $caption = $caption."[".$cr['code']." ".$cr['amount']."]  ";
                                                ...
                                                <input type="button" onClick="javascript: update_staff_rate(<?php echo $userid; ?>);" value="Save Staff Rate Changes" class="button">
                                                </td>
                                            </tr>          
                                            ...
                                            <input type="file" name="img" id="img" style="" class="button"><br /><input type="submit" name="picture" id="upload" value="Upload" class="button">          
                                            </td></tr>    
          ...
          <table width="100%">
            <tr>
                          <td valign="top"><b>Ratings</b></td><td valign="top">
                                <select name="star" style="font:10px tahoma;" onChange="javascript: change_rating_update(<?php echo $userid; ?>, this.value);" >
                                    <?php echo $star_rating_report; ?>
                                </select>
                                <br /><font size=1><i>(Ratings : 1 <img src="images/star.png" align="texttop"> = Low potential  ----  5 <img src="images/star.png" align="top"><img src="images/star.png" align="top"><img src="images/star.png" align="top"><img src="images/star.png" align="top"><img src="images/star.png" align="top">=Highest potential)</i></font><br />
                                <div id="star_rating_loader"></div>
              </td>
                        </tr>
            <tr>
                          <td valign="top">
                            ...
                            <td valign="top">
                <select name="applicant_status" style="font:10px tahoma;" onChange="javascript: app_status_update(<?php echo $userid; ?>,<?php echo $_SESSION['admin_id']; ?>,this.value);" >
                  <?php echo $applicant_status; ?>
                </select>
                <br /><strong>Status History:</strong><br />
                <div id="app_status_loader">
                  <?php echo $applicant_status_report; ?>  
                </div>
              </td>
            </tr>  
            <tr>
                          <td valign="top">
                              <b>ASL Interview Counter</b>
              </td>
                            <td valign="top">
                <a href="javascript: asl_interview_counter(<?php echo $userid; ?>); "><font color="#FF0000"><strong><?php echo $asl_interview_counter; ?></strong></font></a>
              </td>
            </tr>                                                                  
          </table>                
                </td>
    ...
    */
    ...
    $counter = 0;
    $query5="SELECT id, agent_no, rate, comments, DATE_FORMAT(date_created,'%D %M %Y') FROM comments WHERE userid=$userid ORDER BY date_created DESC;";
    //echo $query;
    ...
    <?  
      while(list($id, $agent_no, $rate, $comments, $date) = mysql_fetch_array($result5))
      {
        $counter=$counter+1;
        if($rate=="1")
        {
...
<tr><td align=center>
    ...
    <div style='padding:3px; font:12px Arial;background:#E9E9E9; border:#CCCCCC outset 1px;'>
          ...
          <?php
            ...
            {            
          ?>
        <input type="button" name="button" class="button" onClick="location.href='adminaddsubcon.php?search=userid&keyword=<?php echo $userid;?>'" value="Become a Staff">
                      ...
                      <table width=100% border=0 cellspacing=1 cellpadding=3 bgcolor="#E9E9E9">
                        <tr>
                          ...
                          <td>Date Created</td>
                            ...
                            {
                              echo "<tr>";
                              echo "<td width=0 bgcolor=#ffffff><div id='action".$id."'><font size=1><a href=\"javascript: action_delete('TOP10',".$id."); \">DELETE</a></font></div></td>";
...
<!--
...

  {
    ...
    newitem+="<input type=\"radio\" name=\"templates\" value=\"signature\" checked > Signature Template";
