...
$sql = $db->select()
	...
	->joinLeft(array('o' => 'gs_job_role_selection') , 'o.gs_job_role_selection_id = g.gs_job_role_selection_id' , array('no_of_job_role' , 'proposed_start_date', 'duration_status' , 'currency','date_created' , 'leads_id'))
	->joinLeft(array('c' => 'currency_lookup') , 'c.id = i.currency' , array('code','sign'))
	->where('i.id = ?', $order_id);
*/
$sql = $db->select()
	->from(array('g' => 'gs_job_role_selection') , array('no_of_job_role' , 'proposed_start_date', 'duration_status' , 'currency','date_created' , 'leads_id'))	
	->join(array('l' => 'leads') , 'l.id = g.leads_id',array('fname','lname','email'))
	->join(array('c' => 'currency_lookup') , 'c.code = g.currency' , array('code','sign'))
	->where('g.gs_job_role_selection_id = ?', $order_id);
//echo $sql;
...
$description = "Recruitment Service Order";//$order['description'];
$status = "Incomplete";//$order['status'];
$gs_job_role_selection_id = $order_id;
$fname = $order['fname'];
$lname = $order['lname'];
$email = $order['email'];
