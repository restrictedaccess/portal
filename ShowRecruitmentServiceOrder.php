...
include 'get_started/lib/get_started_function.php';
$smarty = new Smarty();
...
$sql = $db->select()
	...
	->joinLeft(array('o' => 'gs_job_role_selection') , 'o.gs_job_role_selection_id = g.gs_job_role_selection_id' , array('no_of_job_role' , 'proposed_start_date', 'duration_status','indian_applicant', 'filled_up_by_id', 'filled_up_by_type', 'filled_up_date'))
	->joinLeft(array('c' => 'currency_lookup') , 'c.id = i.currency' , array('code','sign'))
	->where('i.id = ?', $order_id)
	->where('SHA1(i.date_created) = ?', $_REQUEST["qk"]);

	
try{
	$order = $db->fetchRow($sql);
} catch(Exception $e){
	echo "The page you are accessing is restricted.";
	//header("Refresh:5; url=/portal/");
	header('Refresh:3 ; URL=/portal/');
...
$code = $order['code'];
$sign = $order['sign'];
$indian_applicant = $order['indian_applicant'];
...
$smarty->assign('indian_applicant', $indian_applicant);
$smarty->assign('filled_up_by_type', $jobrole['filled_up_by_type']);
$smarty->assign('filled_up_date', $order['filled_up_date']);
if($order['filled_up_by_id'] == ""){
    $order['filled_up_by_id'] = $leads_id;
}
$smarty->assign('filled_up_by', ShowFilledBy($order['filled_up_by_id'], $order['filled_up_by_type'], 'name'));
