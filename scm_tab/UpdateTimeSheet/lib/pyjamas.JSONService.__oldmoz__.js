...
pyjamas.JSONService.JSONService = (function(){
	...
	cls_definition.__sendNotify = pyjs__bind_method(cls_instance, '__sendNotify', function(method, params) {
		...
		if (pyjslib.bool(!(pyjamas.JSONService.HTTPRequest().asyncPost((typeof self.url == 'function' && self.__is_instance__?pyjslib.getattr(self, 'url'):self.url), msg_data, self)))) {
			return -1;
		}
		return 1;
	}
	...
	cls_definition.__sendRequest = pyjs__bind_method(cls_instance, '__sendRequest', function(method, params, handler) {
		...
		msg_data = self.parser.encode(msg);
		request_info = pyjamas.JSONService.JSONRequestInfo(id, method, handler);
		if (pyjslib.bool(!(pyjamas.JSONService.HTTPRequest().asyncPost((typeof self.url == 'function' && self.__is_instance__?pyjslib.getattr(self, 'url'):self.url), msg_data, pyjamas.JSONService.JSONResponseTextHandler(request_info))))) {
			return -1;
		}
		return id;
	}
...
pyjamas.JSONService.JSONResponseTextHandler = (function(){
	...
	cls_definition.onCompletion = pyjs__bind_method(cls_instance, 'onCompletion', function(json_str) {
		...
		if (pyjslib.bool(!(response))) {
			...
			self.request.handler.onRemoteError(0, error, (typeof self.request == 'function' && self.__is_instance__?pyjslib.getattr(self, 'request'):self.request));
		}
		else {
