...
function setDefault(obj){
	// obj = the element to set to zero
...
var chatwinhandler = null;
/* 2011-02-02 - mike
  check first the window if exists before opening new one,
  or else focus the existing.
*/
function popup_win8( loc, wd, hg ) {
   if (!chatwinhandler || chatwinhandler.closed){
	var xpos = screen.availWidth - wd; 
	var ypos = screen.availHeight/2 - hg ;
	chatwinhandler = window.open(loc,"newwindow",'width=' + wd + ',height=' + hg + ',resizable=no,toolbar=no,scrollbars=yes,location=0,status=1,screenX=0,screenY=0,top='+ypos+',left='+xpos+'\'');
	
	try{
		 chatwinhandler.focus();
	 } 
	 catch(e){ 
		 alert('The page you clicked could not be opened!\nDo you have any pop-up blocker installed?\n\nIf so, please disable the pop-up blocker or \nadd this site to the "allow pop-up" list.');
	 }
