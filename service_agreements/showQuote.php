...
<?php 
  ...
  */
  while(list($work_status, $work_position, $working_hours, $days, $currency, $quoted_price, $gst, $userid)=mysql_fetch_array($result))
  {
  		$counter++;
		if($currency == "AUD"){
			$currency_symbol = "\$ ";
		...
		if($userid){
			...
			//echo $sql;
			//$personal = mysql_query($sql);
			$personal = mysql_fetch_array(mysql_query($sql));
	...
	<div style="margin-bottom:10px;"><?php echo $counter;?>)&nbsp;&nbsp;<?php echo getWorkStatusLongDescription($work_status);?>&nbsp;<?php echo $work_position;?> <span style=""><?php echo ucwords($candidate);?></span> , working <?php echo $working_hours;?> hours a day, <?php echo $days;?>  days a week </div>
