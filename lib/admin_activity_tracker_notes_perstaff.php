<?
function activity_tracker_notes_perstaff($userid, $admin_id, $method_type)
{
	...
	//get admin email
	$query="SELECT admin_email FROM admin WHERE admin_id='$admin_id' LIMIT 1;";
	$result=mysql_query($query);
	while(list($admin_email) = mysql_fetch_array($result))
	{
	...
	if($is_send == "yes")
	{
		...
		$b_ = date("Y-m-d",$b_1);
		$title = date("M d, Y");
		$query = "SELECT expected_time, checked_in_time, note FROM activity_tracker_notes WHERE userid='$mystaff' AND (DATE_FORMAT(expected_time,'%Y-%m-%d')='$a_') ORDER BY expected_time ";
		$result=mysql_query($query);
		//ended
		...
		<table width="100%"  cellspacing="0" cellpadding="3" style="border:#CCCCCC solid 1px; margin-left:10px; margin-top:20px; margin-bottom:20px;">
		  <tr bgcolor="#CCCCCC">
			<td height="21" colspan="3" valign="middle" style="border-bottom:#666666 solid 1px;"><font color="#000000"><b>Activity Tracker Notes</b><i>('.$title.')</i></font></td>
		  </tr>
		  <tr>
			<td></td>
		  </tr>
